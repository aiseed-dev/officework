...
fn to_table(s: &Sheet) -> Table {
    ...
    // 範囲なので、空のセルどうしの結合はその外に出ます。書かないと
...
/// セル1つ。**セルの中の改行は「行の折り」で、段落の切れ目ではありません**
/// (2026-08-31 発注者)。
///
/// `Document::plain` は改行で段落に割ります。表計算のセルでそれをすると、
/// 「区　分 / Type」の1セルが2段落になり、間が段落の空きだけ開きます。
...
mod tests {
    ...
    /// **字下げが往復で消えない。**
    ///
    /// 2026-08-31 発注者「字下げの往復を直して」。日本の帳票は全角や半角の
    /// 空白で字下げします。読む側はセルの頭の空白を飾りとして落とすので、
    /// `{empty}` を置いて「ここからは字」と示します。
    ///
    /// **頭に空白があれば数にしません。** 「    2020」は字下げした見出しで
    /// あって数ではありません。数にすると空白ごと消えます。
    #[test]
    fn an_indent_survives_the_round_trip() {
        let mut b = Book::new();
        b.sheets.clear();
        let mut s = Sheet::new("覚え");
        for (a1, moji) in [
            ("A1", "　　　　百万円"),   // 全角の字下げ
            ("B1", "    2020"),         // 半角の字下げ + 数に見える字
            ("C1", "ふつう"),
            ("A2", "2020"),             // 字下げの無い数はそのまま数
        ] {
            s.set(Pos::parse(a1).unwrap(), Cell::input(moji));
        }
        b.sheets.push(s);
        let (back, _) = parse(&write(&b)).expect("読めない");
        assert_eq!(value(&back, 0, "A1"), "　　　　百万円", "全角の字下げが消えた");
        assert_eq!(value(&back, 0, "B1"), "    2020", "半角の字下げが消えた(数に化けた)");
        assert_eq!(value(&back, 0, "C1"), "ふつう", "隣のセルが壊れた");
        assert_eq!(value(&back, 0, "A2"), "2020", "字下げの無い数まで字にした");
    }

    /// **幅の広いセルは、入る行から始まる。**
    ///
    /// 2026-08-31。桁の数で行に切るとき、入れると溢れるセルもそのまま入れて
    /// いました。11 セルの後ろに幅 10 のセルが来ると1行に 21 桁ぶん詰まり、
    /// **そこから後ろが1行ずつずれます**(国税庁の酒税の表で 47 セル)。
    #[test]
    fn a_wide_cell_starts_a_row_it_fits_in() {
        // 3列の表(桁の数は1行目で決まります)。2行目は幅3のセル1つで、
        // 3行目が続きます。**セルは流れで並ぶ**ので、幅を見ずに詰めると
        // 「まとめ」が1行目の後ろに食い込みます
        // 2行目はセル1つだけ。次に来る幅3のセルは**入らない**ので、
        // 3行目の頭になります
        let src = ".表\n|===\n|あ |い |う\n|え\n3+|まとめ\n|か |き |く\n|===\n";
        ...
        assert_eq!(value(&b, 0, "A3"), "まとめ", "幅の広いセルが前の行に食い込んだ");
    ...
    fn a_trailing_space_survives_the_round_trip() {
        ...
        assert_eq!(value(&back, 0, "B1"), "隣", "隣のセルが壊れた");
    }
...
Type", "改行が戻らない");
        assert_eq!(value(&back, 0, "B1"), "隣", "隣のセルが次の行へ落ちた");
    }
    ...
    /// **セルの中の縦棒で行が割れない**(2026-08-20 に見つけた)。
    ///
    /// `|` はセルの切れ目そのものです。逃がさずに書くと、`A|B` の1セルが
    /// 2セルに割れて**その行から右がずれます**。`|===` を含むセルなら、
    ...
    fn a_bar_inside_a_cell_does_not_split_the_row() {
        for content in ["A|B", "|見出し\n|中身", "|===\nおしまい", "|"] {
            ...
            assert_eq!(value(&back, 0, "B1"), "番人", "隣のセルまで壊れた:\n{src}");
        }
    }
