...
mod list_tests {
    ...
    fn a_header_pushes_the_body_down_when_the_margin_is_short() {
        ...
        assert_eq!(hf_push_mm(&hf, &pg, None, None, 10.5, false), 12.0, "ヘッダーが無いのに押した");
        hf.paragraphs.push(Paragraph::default());
        let oshita = hf_push_mm(&hf, &pg, None, None, 10.5, false);
        let takasa = 10.5 * 1.292 * 25.4 / 72.0;
        assert!((oshita - (15.0 + takasa)).abs() < 0.05, "距離 + 高さになっていない: {oshita}");
        // 余白のほうが広ければ余白のまま
        pg.top_mm = 35.0;
        assert_eq!(hf_push_mm(&hf, &pg, None, None, 10.5, false), 35.0);
    }
    ...
    fn full_width_characters_sit_on_the_character_grid() {
        ...
        assert!((w[0] - (sizen("あ", 10.0) + aki)).abs() < 0.001, "全角に空きが足されていない: {w:?}");
        assert!((w[1] - sizen("a", 10.0)).abs() < 0.001, "半角まで空きが付いた: {w:?}");
        ...
        let mut hf = HeadFoot::default();
        hf.paragraphs.push(Paragraph::default());
        assert_eq!(hf_push_mm(&hf, &pg, None, None, 10.5, false), 20.0, "固定の余白を押した");
    }
...
mod byte0_tests {
    ...
    /// **全角スペースの字下げが、全角の幅で出る。**
    ///
    /// 2026-08-30、内閣府の告知書で見つけました。日本の書類は `w:ind` を
    /// 使わず全角スペースで字下げすることが多く、この文書も宛名の前に9字
    /// 置いていました。2つの穴が重なっていました。
    ///
    /// . 段落の1行目の頭の空白まで「行頭の空白」として捨てていた
    /// . 捨てずに残しても、セルには**半角スペースを入れて**いた。紙に
    ///   出すときは字を繋げて1つの塊で書くので、送りが書体の半角の幅
    ///   (12pt に対して 3.48pt)になり、9字で 76.7pt ずれていました
    #[test]
    fn an_ideographic_space_indents_by_its_full_width() {
        let ls = lines("\u{3000}\u{3000}\u{3000}様", 100.0);
        let cs = &ls[0].cells;
        assert_eq!(cs.len(), 4, "字下げの空白が落ちた: {cs:?}");
        assert_eq!(cs[0].ch, '\u{3000}', "半角スペースに置き替わっている");
        // 「様」が全角3つぶん右に居ること(幅は書体で決まるので、字の幅で見る)
        let zen = cs[3].w_mm;
        assert!((cs[3].x_mm - zen * 3.0).abs() < 0.05,
                "字下げが全角3つぶんでない: x={} 全角={}", cs[3].x_mm, zen);
        // 折り返した行の頭の空白は、今までどおり捨てる
        let ls = lines("あああ ええええ", 12.0);
        assert!(ls.len() >= 2, "折り返していない");
        assert_ne!(ls[1].cells[0].ch, ' ', "折り返した行の頭に空白が残った");
    }
