...
mod table_layout_tests {
    ...
    fn セルの中で折り返す() {
        ...
        let mut d = Document { note_ids_taken: Vec::new(), template: None, styles: Vec::new(), styles_new: Vec::new(),  footnote_fmt: Default::default(), size_pt: None, endnote_fmt: Default::default(), font: None, page: None, sect_raw: None, footnotes: Vec::new(), header: Default::default(), footer: Default::default(), page_color: None, watermark: None, ink: Vec::new(), track_author: None, hyphenate: false, protection: None, props: Default::default(), vertical: false, blocks: vec![] };
        d.blocks.push(Block::Table(Table {
            col_mm: vec![],
...
mod merge_layout_tests {
    ...
    fn sheet_of(rows: Vec<Vec<Cellbox>>) -> Sheet {
        let data = font::load(font::for_document(None).unwrap().0).unwrap();
        let m = Metrics::new(&data).unwrap();
        let d = Document { note_ids_taken: Vec::new(), template: None, styles: Vec::new(), styles_new: Vec::new(),  footnote_fmt: Default::default(), size_pt: None, endnote_fmt: Default::default(),
