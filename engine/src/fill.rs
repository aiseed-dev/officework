...
//! ```
//!
//! `{名前}` はそのまま置き換えます(AsciiDoc の属性の参照と同じ書き方です。
//! 前からの `{{名前}}` も受けます)。`{群.項目}` を含む**表の行は、その群の
...
fn names(s: &str) -> Vec<(usize, usize, String)> {
    ...
    while i < b.len() {
        if b[i] == b'{' {
            // **本家の書き方 `{名前}` を正とします**(2026-08-18 発注者
            // 「AsciiDoc とは何かを考えていけば理解できてくるのでは」)。
            // AsciiDoc は属性の参照を `{名前}` と書きます。うちの差し込みは
            // `{{名前}}` という別の書き方を作っていたので、本家に寄せました。
            // **`{{名前}}` も今までどおり受けます** — 手引きと見本が
            // その書き方で出ているためです
            let 二重 = b.get(i + 1) == Some(&b'{');
            let 頭 = if 二重 { i + 2 } else { i + 1 };
            let 閉じ = if 二重 { "}}" } else { "}" };
            if let Some(rel) = s[頭..].find(閉じ) {
                let name = s[頭..頭 + rel].trim().to_string();
