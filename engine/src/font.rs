...
/// | Meiryo UI | 1.679em | 内閣府 document_4.pdf。14pt→23.5pt |
///
/// どれも `w:spacing w:line` の指定が無く、文字グリッドも効いていない
/// 文書で測っています。指定がある文書は書体の行送りを上書きするので、
/// ここの値には使えません。
///
/// Meiryo UI は一度 1.966em で入れて外しました。あの値は `docDefaults` の
/// 1.15 倍が混ざっていたためです。いまの 1.679em は、スタイル Normal が
/// `w:line="240"`(1倍)と言っている表の行から測っています
/// (1.679 × 1.15 = 1.93 で、外した値とも辻褄が合います)。
///
