...
///
/// ＭＳ Ｐ明朝とＭＳ Ｐゴシックは、漢字が全角のセルに収まり、**半角の数字が
...
/// **ＭＳ 明朝とＭＳ ゴシックの行送り(em)。**
///
/// **この2つだけの特例です**(2026-09-01 発注者「ＭＳゴシック、ＭＳ明朝の
/// 特例ということにしませんか」)。日本の役所の様式はこの2つで作られて
/// いて、どちらもこの機械には無く、置き替え先とは縦の寸法が違います
/// (梅明朝は 1.0000em、ＭＳ 明朝は 1.292em)。
///
/// | 原本 | 行送り | どこで測ったか |
/// |---|---|---|
/// | ＭＳ 明朝 | 1.292em | 内閣府 chosahyo.pdf。12pt→15.54pt、10.02pt→12.96pt |
/// | ＭＳ ゴシック | 1.292em | ＭＳ 明朝と同じ値を当てます(下の断り) |
/// | Century | 1.360em | 国税庁の酒税の xlsx を Excel が刷った PDF |
///
/// ＭＳ 明朝は `w:spacing w:line` の指定が無く、文字グリッドも効いていない
/// 文書で測っています。指定がある文書は書体の行送りを上書きするので、
/// ここの値には使えません。
///
/// **ＭＳ ゴシックは別に測っていません。** ＭＳ 明朝と同じ値を当てる、と
/// 決めた物です(2026-09-01 発注者)。手元に原本が無く、この2つが同じ
/// 寸法かどうかは確かめられていません。
///
/// Meiryo UI は一度 1.966em、次に 1.679em で入れましたが外しました
/// (2026-09-01 発注者「Meiryo は、もうやめておきましょう」)。表に無い
/// 書体は、この機械にある置き替え先そのものから出します。
///
...
const OKURI_EM: &[(&str, f32)] = &[
    ...
    ("ＭＳゴシック", 1.292),
    ("msgothic", 1.292),
    // 「Ｐ」つきと UI は同じファイル(msmincho.ttc / msgothic.ttc)で縦の寸法も同じ
    // (2026-09-09、xlsx 側の作業者の報告)
    ("ＭＳＰ明朝", 1.292),
    ("mspmincho", 1.292),
    ("ＭＳＰゴシック", 1.292),
    ("mspgothic", 1.292),
    ...
    ("msuigothic", 1.292),
    ("century", 1.360),
    // **ＭＳ の代替に立つ書体は、行送りも原本の値**(2026-09-09)。run の書体は
    // 描く前に代替の名前(梅明朝など)に解決されるので、ここに無いと梅明朝の
    // 1.000em で組み、Word より 3pt 低い行になる(厚労省の様式の表の中。
    // Opus Mac が切り分けた)。梅・IPA は ＭＳ の代わりに入れる書体なので、
    // ＭＳ 明朝を測った 1.292em を当てる
    ("梅明朝", 1.292),
    ("UmeMincho", 1.292),
    ("梅ゴシック", 1.292),
    ("UmeGothic", 1.292),
...
fn okuri_em_yomu(name: Option<&str>) -> Option<f32> {
    ...
    // 1em を下回ることがあり(Meiryo UI の受けで 0.684em になっていました)、
    // 行が字より低いと、同じ行の別のセルが決めた高さの中で字が上に浮き、
    // 下の罫線との間が空きます(2026-09-01 発注者「取るべき行動項目は
    // アンダーラインとの間に空白がある」)
