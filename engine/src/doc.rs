...
pub struct Table {
    ...
    pub row_mm: Vec<f32>,
    /// **行の高さが固定か**(docx の `w:trHeight w:hRule="exact"`)。`row_mm` と同じ並び。
...
pub struct HeadFoot {
    ...
    pub anchors: Vec<String>,
    /// **表を持つヘッダー・フッターの中身**(2026-09-20)。`paragraphs` は
    /// 段落を平らに並べた物で、表の枠と欄の位置を持てません。表のある部品
    /// (Word のテンプレートの多くが日付・題・頁番号を 1 行の表に入れます)は
    /// ここに元のブロックをそのまま持ち、[`crate::layout_hf`] が本文と同じ
    /// 組み方で組みます。表の無い部品では空です
    pub blocks: Vec<Block>,
}
