...
def main(page: ft.Page):
    ...
    goal = section(
        ...
        md("\n".join([
            ...
            "* **Microsoft のテンプレートが、そのまま使える。** Word の「新規」に出る物を、そのまま開いて、直して、刷る。",
            "* **官庁の文書を、そのまま編集できる。** 省庁が公開している申請書や届出書を、同じに開いて、書き込んで、返す。",
            "* **原稿は文字(AsciiDoc)で持てる。** git に入れるのは原稿だけで、デザインはテンプレートに分けて git に入れない。docx と PDF に書き出せる。",
            "* **組織の間のやり取りも、いずれ文字(AsciiDoc)で足りる。** docx は受け渡しの形式として残り、中身は文字でやり取りする。",
