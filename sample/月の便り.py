...
#
# - **会員登録で同意をもらう**(発注者 2026-08-15「会員登録してもらえば
#   いいのでしょう」)。過去に買った人は「取引関係にある者」でオプトインの
#   例外に当たりうるが、案内が主目的なら規制が掛かる。**申込をもらうのが
#   いちばん確か**で、申込そのものが証跡になる
# - **同意の証跡が無ければ作らない。** 法は「いつ・どうやって同意したか」の
#   記録の保存を求めている。台帳の「案内可 ○」だけでは足りず、
#   「同意した日」と「同意の取り方」が要る。**外した人と理由を必ず言う**
...
# **同意の証跡が要る**(特定電子メール法。同意を証する記録の保存義務)。
# 「案内可 ○」だけでは足りない — **いつ・どうやって同意したか**を残す
台帳の見出し = ["番号", "お名前", "メール", "ご住所", "最後の注文",
                "案内可", "同意した日", "同意の取り方", "断った日", "覚え書き"]

見本の客 = [
    ("山田 花子", "hanako@example.invalid", "〒000-0000 どこかの県どこかの市1-2-3",
     "2026-08-15", "○", "2026-04-02", "会員申込の頁", "", "青しそと聖護院かぶ"),
    ("鈴木 一郎", "ichiro@example.invalid", "〒111-1111 となりの県となりの市4-5-6",
     "2026-07-02", "○", "2026-05-18", "申込書(紙)", "", "豆をまとめて"),
    ("佐藤 みどり", "midori@example.invalid", "〒222-2222 遠くの県遠くの市7-8-9",
     "2026-05-20", "", "", "", "", "**会員でない** — 作らない"),
    ("高橋 太郎", "taro@example.invalid", "〒333-3333 むこうの県むこうの市1-1-1",
     "2026-03-11", "○", "2026-01-09", "会員申込の頁", "2026-06-30",
     "**断られた** — 作らない"),
    ("田中 実", "minoru@example.invalid", "〒444-4444 あちらの県あちらの市2-2-2",
     "2026-08-01", "○", "", "", "", "**○ だけで証跡が無い** — 作らない"),
]
...
def 客の台帳を作る():
    ...
    for col, w in (("A", 8), ("B", 16), ("C", 28), ("D", 34), ("E", 14),
                   ("F", 10), ("G", 14), ("H", 18), ("I", 12), ("J", 30)):
...
def 申込の頁を作る():
    """**会員登録の口。** ここで同意をもらうのがいちばん確かで、
    申込そのものが「いつ・どうやって同意したか」の証跡になる
    (発注者 2026-08-15「会員登録してもらえばいいのでしょう」)。

    **JavaScript は使わない。** form の POST は JS 以前からの素の仕組み。
    受け口は catalog_server.py と同じ形で足せる(POST /join)。
    """
    出 = [
        "<!doctype html>", '<html lang="ja"><head><meta charset="utf-8">',
        '<meta name="viewport" content="width=device-width,initial-scale=1">',
        "<title>会員のお申し込み</title><style>",
        "body{font-family:sans-serif;max-width:34em;margin:2em auto;padding:0 1em;line-height:1.8}",
        "label{display:block;margin:.8em 0 .2em}",
        "input[type=text],input[type=email]{width:100%;padding:.5em;font-size:1em}",
        "button{margin-top:1.2em;padding:.6em 1.6em;font-size:1em}",
        ".note{background:#f6f6f2;padding:1em;border-left:4px solid #1b6e3c}",
        "</style></head><body>",
        f"<h1>{差出人['名前']} 会員のお申し込み</h1>",
        "<p>月に一度、その月にお出しできる種と畑の様子をお送りします。</p>",
        '<form method="post" action="/join">',
        '<label>お名前<input type="text" name="名前" required></label>',
        '<label>メール<input type="email" name="メール" required></label>',
        '<label>ご住所<input type="text" name="住所"></label>',
        '<label><input type="checkbox" name="案内可" value="○" required> '
        "月に一度のお便りを受け取ります</label>",
        '<button type="submit">申し込む</button>',
        "</form>",
        '<div class="note">',
        "<p><b>いただいた住所とメールは、お便りとお届けにだけ使います。</b>"
        "他へ渡しません。</p>",
        "<p>やめたいときは、お便りに「不要」と返信していただければ"
        "以後お送りしません。</p>",
        f"<p>{差出人['名前']} / {差出人['住所']} / "
        f"電話 {差出人['電話']} / {差出人['メール']}</p>",
        "</div>", "</body></html>",
    ]
    申込の頁.write_text("\n".join(出), encoding="utf-8")


def 申込を受ける(名前, メール, 住所, 受けた日, 取り方="会員申込の頁"):
    """申込を台帳に足す。**同意した日と取り方をここで残す**
    — 後から「同意をもらった証拠は」と聞かれたときに出せるのはこれ"""
    b = sheet.Book.open(客の台帳)
    ws = b[b.sheet_names[0]]
    for r in ws.values()[1:]:
        if len(r) > 2 and r[2] == メール:
            return False, f"{名前}: すでに台帳にあります"
    r = ws.max_row + 1
    for c, v in enumerate([f"{r - 1:03d}", 名前, メール, 住所, "",
                           "○", 受けた日, 取り方, "", ""], start=1):
        ws.cell(row=r, column=c).value = v
    b.save(客の台帳)
    return True, f"{名前}: 台帳に足しました(同意 {受けた日} / {取り方})"
...
def 客を読む():
    ...
    for r in ws.values()[1:]:
        (番号, 名前, メール, 住所, 最後, 案内可,
         同意日, 取り方, 断った日, _) = (list(r) + [None] * 10)[:10]
        if not 名前:
            continue
        if 断った日:
        ...
        if str(案内可 or "").strip() not in ("○", "◯", "はい", "yes", "y"):
            外した.append(f"{名前}: 会員ではありません(案内の同意なし)")
            continue
        # **証跡の無い同意は同意でない。** 法は「いつ・どうやって同意したか」の
        # 記録の保存を求めている。○ だけ付いていても送らない
        if not 同意日 or not 取り方:
            外した.append(f"{名前}: 同意の記録(日と取り方)がありません")
            continue
        if not メール:
...
if __name__ == "__main__":
    ...
    申込の頁を作る()
    print(f"会員申込の頁: {申込の頁.name}")

    # 申込が1件来た体(実物では POST /join がこれを呼ぶ)
    足りた, 言い分 = 申込を受ける(
        "中村 さくら", "sakura@example.invalid",
        "〒555-5555 そちらの県そちらの市3-3-3", "2026-08-15")
    print(f"  {言い分}")
