...
def main() -> None:
    ...
    引数はありません。前は `--panel`(エージェントのパネルから起こされた印)で
    run_macro を足していましたが、パネルは MCP を渡さない形になりました
    (2026-09-08。Claude Code 自身の道具で文書のファイルを直し、表は Python で
    つなぐ)。任意のコードを走らせる道具は外の AI に置きません。
    """
    import sys
