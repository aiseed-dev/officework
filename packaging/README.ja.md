...
表と文書は同じウィンドウのタブで開きます。ファイルを続けて渡しても、
ウィンドウは増えずにタブが増えます。
...
見本を置き場にコピーして、officework を開き直します。
...
手続きなので officework の中から呼びます。家計簿は AI が要ります(詳しくは
...
- **セルの関数が `#PY?` のまま** — 置き場を確かめてください
  (`~/.config/officework/plugins/*.py`)。置いた後は officework を開き直します
