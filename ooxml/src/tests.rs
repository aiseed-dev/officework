...
mod round {
    ...
    /// **段落自身の `w:ind` は 0 でも指定**(2026-09-09)。読み手は言った印を立てる
    #[test]
    fn a_paragraphs_own_indent_is_marked_even_when_zero() {
        let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:pPr><w:pStyle w:val="a3"/><w:ind w:leftChars="0"/></w:pPr><w:r><w:t>甲</w:t></w:r></w:p><w:p><w:pPr><w:pStyle w:val="a3"/></w:pPr><w:r><w:t>乙</w:t></w:r></w:p></w:body></w:document>"#;
        let (d, _) = crate::read::parse_document_xml(xml);
        let ps: Vec<&kumihan::Paragraph> = d.paragraphs().collect();
        assert!(ps[0].ind_itta && ps[0].left_twips == 0, "0 の指定が印にならない");
        assert!(!ps[1].ind_itta, "言っていない段落に印が付いた");
    }

    /// **run の書体は字の種類で選ぶ**(2026-09-09)。和文を含む run は `w:eastAsia`
    /// (無ければ文書の既定)、半角だけの run は `w:ascii` / `w:hAnsi`。
    /// `w:hint="eastAsia"` の run は半角も和文の書体。右のインデントも読む
    #[test]
    fn a_run_font_follows_the_script_of_its_text() {
        let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:pPr><w:ind w:right="420"/></w:pPr><w:r><w:rPr><w:rFonts w:hAnsi="Times New Roman"/></w:rPr><w:t>　なお、</w:t></w:r><w:r><w:rPr><w:rFonts w:hAnsi="Times New Roman"/></w:rPr><w:t>2024</w:t></w:r><w:r><w:rPr><w:rFonts w:hAnsi="Times New Roman" w:hint="eastAsia"/></w:rPr><w:t>(1)</w:t></w:r><w:r><w:rPr><w:rFonts w:ascii="Century" w:eastAsia="ＭＳ ゴシック"/></w:rPr><w:t>甲</w:t></w:r></w:p></w:body></w:document>"#;
        let (d, _) = crate::read::parse_document_xml(xml);
        let p = d.paragraphs().next().unwrap();
        let f: Vec<Option<&str>> = p.runs.iter().map(|r| r.font.as_deref()).collect();
        assert_eq!(f[0], None, "和文の run が欧文の書体になった");
        assert_eq!(f[1], Some("Times New Roman"), "半角だけの run は欧文の書体");
    ...
    /// `w:ind` が無い `w:numPr` の段落は、段の `w:ind` を既定として持つ
    #[test]
    fn a_numbered_paragraph_takes_its_indent_from_the_numbering_level() {
        let num = r#"<w:numbering xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:abstractNum w:abstractNumId="0"><w:lvl w:ilvl="0"><w:start w:val="1"/><w:numFmt w:val="decimal"/><w:lvlText w:val="%1."/><w:pPr><w:ind w:left="420" w:hanging="420"/></w:pPr></w:lvl></w:abstractNum><w:num w:numId="1"><w:abstractNumId w:val="0"/></w:num></w:numbering>"#;
        let hyou = crate::read::num_indents(num);
        assert_eq!(hyou.get(&(1, 0)), Some(&(420, 420, false)), "段の字下げが引けない: {hyou:?}");
        let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:pPr><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr></w:pPr><w:r><w:t>建築物の名称</w:t></w:r></w:p><w:p><w:pPr><w:numPr><w:ilvl w:val="0"/><w:numId w:val="1"/></w:numPr><w:ind w:left="840" w:hanging="210"/></w:pPr><w:r><w:t>自分で言う</w:t></w:r></w:p></w:body></w:document>"#;
        let (d, _) = crate::read::parse_document_rels_num(xml, &Default::default(), &Default::default(), &Default::default(), &crate::read::num_markers(num), &hyou);
        let ps: Vec<&kumihan::Paragraph> = d.paragraphs().collect();
        assert_eq!((ps[0].left_twips, ps[0].first_line_twips), (420, -420), "段の字下げが段落に当たらない");
        assert_eq!((ps[1].left_twips, ps[1].first_line_twips), (840, -210), "段落自身の w:ind が負けた");
    }

    /// **`w:numId w:val="0"` is read as "numbering removed", and written
    /// back** (ECMA-376 17.9.19). Read as "no numbering" alone, the style's
    /// bullet came back, and a saved file lost the removal (2026-09-23)
    #[test]
    fn numbering_removed_by_num_id_zero_is_read_and_written_back() {
        let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:pPr><w:pStyle w:val="TipTextBullet"/><w:numPr><w:ilvl w:val="0"/><w:numId w:val="0"/></w:numPr></w:pPr><w:r><w:t>Cash</w:t></w:r></w:p><w:p><w:pPr><w:pStyle w:val="TipTextBullet"/></w:pPr><w:r><w:t>Petty</w:t></w:r></w:p></w:body></w:document>"#;
        let (d, _) = crate::read::parse_document_xml(xml);
        let ps: Vec<&kumihan::Paragraph> = d.paragraphs().collect();
        assert!(ps[0].list_off, "numId 0 を読んでいない");
        assert!(!ps[1].list_off, "numPr の無い段落まで番号を外した");
        let mut out = Vec::new();
        crate::write(&d, std::io::Cursor::new(&mut out)).expect("書けない");
        let (d2, _) = crate::read(std::io::Cursor::new(out)).expect("読み直せない");
        let ps2: Vec<&kumihan::Paragraph> = d2.paragraphs().collect();
        assert!(ps2[0].list_off, "保存で numId 0 が消えた");
    ...
    /// 塗りが出ていませんでした(2026-09-03 から 2026-09-21 まで)
    #[test]
    fn a_cell_takes_the_fill_of_an_empty_shd() {
        let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:tbl><w:tblGrid><w:gridCol w:w="2000"/></w:tblGrid><w:tr><w:tc><w:tcPr><w:tcW w:w="2000" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="595959" w:themeFill="text1"/></w:tcPr><w:p><w:r><w:t>題</w:t></w:r></w:p></w:tc></w:tr></w:tbl></w:body></w:document>"#;
        let (d, _) = crate::read::parse_document_xml(xml);
        let t = d.tables().next().expect("表がありません");
    ...
    fn a_cell_takes_the_fill_of_a_shd_with_a_closing_tag() {
        let xml = r#"<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:tbl><w:tblGrid><w:gridCol w:w="2000"/></w:tblGrid><w:tr><w:tc><w:tcPr><w:tcW w:w="2000" w:type="dxa"/><w:shd w:val="clear" w:color="auto" w:fill="EAEAEA"></w:shd></w:tcPr><w:p><w:r><w:t>日付</w:t></w:r></w:p></w:tc></w:tr></w:tbl></w:body></w:document>"#;
        let (d, _) = crate::read::parse_document_xml(xml);
        let t = d.tables().next().expect("表がありません");
        assert_eq!(t.rows[0][0].shade.as_deref(), Some("EAEAEA"), "閉じ札つきの塗りが読めていない");
    }
    ...
    fn a_nested_table_keeps_the_outer_cell_span() {
        ...
        // 中の表は「表を持つ段落」として外側のセルの段落の列に入る(2026-09-09)
        let text: Vec<String> = t.rows[0][0].paragraphs.iter()
            .map(|p| p.runs.iter().map(|r| r.text.as_str()).collect::<String>()).collect();
        assert_eq!(text, vec!["外", "", "後"], "中の表の段落が外側のセルの中に並んでいない");
        let naka = t.rows[0][0].paragraphs[1].nested.as_deref().expect("中の表が段落に無い");
        assert_eq!(naka.rows.len(), 1);
        assert_eq!(naka.rows[0].len(), 2, "中の表のセルが落ちた");
        let uchi: Vec<String> = naka.all_paragraphs().iter()
            .map(|p| p.runs.iter().map(|r| r.text.as_str()).collect::<String>()).collect();
        assert_eq!(uchi, vec!["内1", "内2"]);
    ...
    #[test]
    fn the_document_grid_pitch_is_read_from_the_section() {
        let raw = r#"<w:sectPr><w:pgSz w:w="11906" w:h="16838"/><w:pgMar w:top="1985" w:right="1701" w:bottom="1701" w:left="1701" w:header="851" w:footer="992" w:gutter="0"/><w:cols w:space="425"/><w:docGrid w:type="lines" w:linePitch="360"/></w:sectPr>"#;
        let pg = crate::read::parse_sect(raw);
        assert!((pg.line_pitch_pt - 18.0).abs() < 0.01, "360 twip = 18pt のはず: {}", pg.line_pitch_pt);
        ...
        let (d, _) = crate::read::parse_document_xml(xml);
        let ps: Vec<&kumihan::Paragraph> = d.paragraphs().collect();
        assert!(ps[0].no_grid, "段落の snapToGrid=0 が読めていない");
        assert!(!ps[1].no_grid, "run の snapToGrid を段落の物と取り違えた");
        assert!((d.page.unwrap().line_pitch_pt - 18.0).abs() < 0.01, "文書の用紙に行送りが無い");
    }
...
<w:num w:numId="9"><w:abstractNumId w:val="5"/></w:num>
        ...
        assert_eq!(hyou.get(&(7, 0)), Some(&("○".to_string(), false, None, kumihan::ListNumFmt::Decimal)), "行頭文字が引けない");
        assert_eq!(hyou.get(&(9, 0)), Some(&("(%1)".to_string(), true, None, kumihan::ListNumFmt::Decimal)), "番号の印が引けない");
        assert!(!hyou.contains_key(&(8, 0)), "無い numId に印が出た");
        // 印の `%1` は、その段の番号に置き替わります
        let mut p = kumihan::Paragraph { list: kumihan::ListKind::Number, ..Default::default() };
        p.list_text = Some("(%1)".into());
        assert_eq!(p.marker(2).as_deref(), Some("(3)"), "番号が入らない");
        p.list = kumihan::ListKind::Bullet;
        p.list_text = Some("○".into());
        assert_eq!(p.marker(0).as_deref(), Some("○"), "印がそのまま出ない");
    }
    use super::*;
    use kumihan::{Block, Cellbox, Document, Paragraph, Run, Table};

    fn para(s: &str) -> Paragraph {
        Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, space_before_pt: 0.0, space_after_pt: 0.0,  align: Default::default(), style: Default::default(), comments: Vec::new(), bookmarks: Vec::new(), dropcap: false, anchors: Vec::new(), sect: None,
    ...
    fn font_size_is_preserved() {
        let d = Document { align: None, no_html_auto_space: false, page_start: None, wrap_trail_spaces: false, balance_sbcs: false, font_latin: None, color: None, size_pt: None, theme_colors: Vec::new(), theme_line_pt: Vec::new(), space_after_pt: None, line_spacing: None, note_ids_taken: Vec::new(), template: None, attrs: Vec::new(), styles: Vec::new(), styles_new: Vec::new(),  footnote_fmt: Default::default(), endnote_fmt: Default::default(), font: None, page: None, sect_raw: None, footnotes: Vec::new(), header: Default::default(), footer: Default::default(), page_color: None, watermark: None, ink: Vec::new(), shapes: Vec::new(), track_author: None, hyphenate: false, compress_punct: false, sect_hf: Default::default(), title_pg: false, first_header: None, first_footer: None, protection: None, props: Default::default(), vertical: false, blocks: vec![Block::Para(Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, space_before_pt: 0.0, space_after_pt: 0.0,  align: Default::default(), style: Default::default(), comments: Vec::new(), bookmarks: Vec::new(), dropcap: false, anchors: Vec::new(), sect: None,
                    images: Vec::new(), page_break_before: false, keep_next: false, widow_control: None,
...
mod font_tests {
    ...
    fn font_name_round_trips() {
        ...
        let doc = Document { align: None, shapes: Vec::new(), no_html_auto_space: false, page_start: None, wrap_trail_spaces: false, balance_sbcs: false, font_latin: None, color: None, size_pt: None, theme_colors: Vec::new(), theme_line_pt: Vec::new(), space_after_pt: None, line_spacing: None, note_ids_taken: Vec::new(), template: None, attrs: Vec::new(), styles: Vec::new(), styles_new: Vec::new(),  footnote_fmt: Default::default(), endnote_fmt: Default::default(), footnotes: Vec::new(),
            ...
            blocks: vec![Block::Para(Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, line_pt: None, first_line_chars: None, align_itta: false, tab_stops: Vec::new(), space_before_pt: 0.0, space_after_pt: 0.0,  style: Default::default(), comments: Vec::new(), bookmarks: Vec::new(), dropcap: false, sect: None,
                align: Default::default(),
                anchors: Vec::new(),
                    images: Vec::new(),
                page_break_before: false, keep_next: false, widow_control: None,
                    list: Default::default(),
                indent: 0,
                left_twips: 0, right_twips: 0, first_line_twips: 0,
                line_spacing: 1.0,
...
mod fmt_tests {
    ...
    fn bold_italic_and_underline_round_trip() {
        ...
        let d = Document { align: None, shapes: Vec::new(), no_html_auto_space: false, page_start: None, wrap_trail_spaces: false, balance_sbcs: false, font_latin: None, color: None, size_pt: None, theme_colors: Vec::new(), theme_line_pt: Vec::new(), space_after_pt: None, line_spacing: None, note_ids_taken: Vec::new(), template: None, attrs: Vec::new(), styles: Vec::new(), styles_new: Vec::new(),  footnote_fmt: Default::default(), endnote_fmt: Default::default(), footnotes: Vec::new(),
            ...
            page: None,
            sect_raw: None, header: Default::default(), footer: Default::default(), page_color: None, watermark: None, ink: Vec::new(), track_author: None, hyphenate: false, compress_punct: false, sect_hf: Default::default(), title_pg: false, first_header: None, first_footer: None, protection: None, props: Default::default(), vertical: false,
            blocks: vec![Block::Para(Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, space_before_pt: 0.0, space_after_pt: 0.0,  align: Align::Left, style: Default::default(), comments: Vec::new(), bookmarks: Vec::new(), dropcap: false, anchors: Vec::new(), sect: None,
                    images: Vec::new(), page_break_before: false, keep_next: false, widow_control: None,
                    list: Default::default(), indent: 0, left_twips: 0, right_twips: 0, first_line_twips: 0, first_line_chars: None, align_itta: false, tab_stops: Vec::new(), line_spacing: 1.0, line_pt: None, shade: None, boxed: false, border: Default::default(), images_new: Vec::new(), runs: vec![run("見出し", f.clone())] })],
        };
    ...
    fn strikethrough_and_font_color_round_trip() {
        ...
        let d = Document { align: None, shapes: Vec::new(), no_html_auto_space: false, page_start: None, wrap_trail_spaces: false, balance_sbcs: false, font_latin: None, color: None, size_pt: None, theme_colors: Vec::new(), theme_line_pt: Vec::new(), space_after_pt: None, line_spacing: None, note_ids_taken: Vec::new(), template: None, attrs: Vec::new(), styles: Vec::new(), styles_new: Vec::new(),  footnote_fmt: Default::default(), endnote_fmt: Default::default(), footnotes: Vec::new(),
            ...
            blocks: vec![Block::Para(Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, space_before_pt: 0.0, space_after_pt: 0.0,  align: Align::Left, style: Default::default(), comments: Vec::new(), bookmarks: Vec::new(), dropcap: false, anchors: Vec::new(), sect: None,
                    images: Vec::new(), page_break_before: false, keep_next: false, widow_control: None,
                    list: Default::default(), indent: 0, left_twips: 0, right_twips: 0, first_line_twips: 0, first_line_chars: None, align_itta: false, tab_stops: Vec::new(), line_spacing: 1.0, line_pt: None, shade: None, boxed: false, border: Default::default(), images_new: Vec::new(), runs: vec![run("赤", f.clone())] })],
        };
...
/// **段落の前後の空きが往復で消えない**(2026-08-15)。
    ...
    fn paragraph_spacing_round_trips() {
        let mut p = Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, space_before_pt: 12.0, space_after_pt: 6.0,
            align: Default::default(), style: Default::default(), comments: Vec::new(),
            bookmarks: Vec::new(), anchors: Vec::new(), sect: None, images: Vec::new(),
            page_break_before: false, keep_next: false, widow_control: None, list: ListKind::None, indent: 0, left_twips: 0, right_twips: 0, first_line_twips: 0, first_line_chars: None, align_itta: false, tab_stops: Vec::new(),
    ...
    fn a_paragraph_without_spacing_stays_plain() {
        let p = Paragraph { toc: false, list_off: false, style_id: None, raw_adoc: None, list_text: None, list_id: None, list_no_tab: false, list_color: None, list_fmt: None, no_grid: false, ind_itta: false, auto_before: None, auto_after: None, before_itta: false, after_itta: false, tate: false, nested: None, space_before_pt: 0.0, space_after_pt: 0.0,
            align: Default::default(), style: Default::default(), comments: Vec::new(),
            bookmarks: Vec::new(), anchors: Vec::new(), sect: None, images: Vec::new(),
            page_break_before: false, keep_next: false, widow_control: None, list: ListKind::None, indent: 0, left_twips: 0, right_twips: 0, first_line_twips: 0, first_line_chars: None, align_itta: false, tab_stops: Vec::new(),
            line_spacing: 1.0, line_pt: None, shade: None, boxed: false, border: Default::default(), dropcap: false,
    ...
    /// **左インデントは twip のまま往復する。**
    ///
    /// 段数(`indent`)は全角2文字きざみなので、1文字(210 twip)が2文字に、
    /// 3文字(630)が4文字になっていました。内閣府の告知書で 123 か所ずれて
    /// いたのを 2026-08-30 に見つけました。開いて保存するだけで字下げが
    /// 動くので、往復でも見ます。
    #[test]
    fn the_left_indent_keeps_its_exact_twips() {
        for tw in [210i32, 220, 426, 440, 630, 840] {
            let mut p = para(ListKind::None, 0, 1.0);
            p.left_twips = tw;
            assert_eq!(roundtrip(p).left_twips, tw, "w:left {tw} が動いた");
        }
    }
    ...
    fn a_word_text_box_is_read_with_all_its_lines() {
        ...
        let a0 = a.replace(
            "</wp:anchor>",
            r#"<wp14:sizeRelH relativeFrom="page"><wp14:pctWidth>0</wp14:pctWidth></wp14:sizeRelH><wp14:sizeRelV relativeFrom="page"><wp14:pctHeight>0</wp14:pctHeight></wp14:sizeRelV></wp:anchor>"#,
        );
        let f0 = crate::foreign_shape(&a0).expect("他所の図形が読めない");
        assert!(f0.w_pct.is_none() && f0.h_pct.is_none(), "0% を大きさにした: {:?}", f0.w_pct);
        let a9 = a0.replace("<wp14:pctWidth>0<", "<wp14:pctWidth>92000<");
        let f9 = crate::foreign_shape(&a9).expect("他所の図形が読めない");
        assert_eq!(f9.w_pct.as_ref().map(|(_, v)| (v * 100.0).round()), Some(92.0));
    }
...
mod default_font_tests {
    ...
    /// **文書の既定の字の色**(`w:docDefaults/w:rPrDefault/w:rPr/w:color`。
    /// ECMA-376 17.3.2.6)。どのスタイルも色を言わない run に届きます。
    /// Word の要旨の型紙は `595959` と書いていて、本文が黒で出ていました
    /// (2026-09-21)
    #[test]
    fn the_document_default_colour_is_read() {
        let styles = r#"<w:styles xmlns:w="x"><w:docDefaults><w:rPrDefault><w:rPr>
            <w:color w:val="595959" w:themeColor="text1" w:themeTint="A6"/><w:sz w:val="22"/>
            </w:rPr></w:rPrDefault></w:docDefaults></w:styles>"#;
        let src = docx(styles, None);
        let (doc, _) = crate::read(Cursor::new(&src)).unwrap();
        assert_eq!(doc.color.as_deref(), Some("595959"), "文書の既定の色が読めていない");
    }

    /// `w:val="auto"` は「色を言わない」ので持ち越しません
    #[test]
    fn an_auto_document_colour_is_not_kept() {
        let styles = r#"<w:styles xmlns:w="x"><w:docDefaults><w:rPrDefault><w:rPr>
            <w:color w:val="auto"/></w:rPr></w:rPrDefault></w:docDefaults></w:styles>"#;
        let src = docx(styles, None);
        let (doc, _) = crate::read(Cursor::new(&src)).unwrap();
        assert_eq!(doc.color, None, "auto を色として読んだ: {:?}", doc.color);
    }
