...
OVERRIDES = {
    ...
    "zh-tw": {
        ...
        "表にする": "轉成表格",
        "要約": "摘要",
        # **「印刷タイトル」と同じラベルになっていました**(2026-08-21)。
        # この言語では「タイトル」と「見出し」が同じ語なので、行番号と
        # 列見出しを印刷するボタンだと分かるところまで書きます。
        # 語は LibreOffice の公式訳(Row & Column Headers)から取りました
        "見出しも印刷": "列印列標和欄標",
        "計算方法": "計算方式",
        "詳細の非表示": "隱藏詳細資料",
        "重複の削除": "移除重複",
    ...
    "pt-br": {
        ...
        # 列見出しを印刷するボタンだと分かるところまで書きます。
        # 語は LibreOffice の公式訳(Row & Column Headers)から取りました
        "見出しも印刷": "Imprimir cabeçalhos de linhas e colunas",
        "計算方法": "Cálculo",
    ...
    "ru": {
        ...
        # 列見出しを印刷するボタンだと分かるところまで書きます。
        # 語は LibreOffice の公式訳(Row & Column Headers)から取りました
        "見出しも印刷": "Печатать заголовки строк и столбцов",
        "計算方法": "Вычисления",
    ...
    "ko": {
        ...
        # 日本語を Excel の「方向」にしたので、訳は本家の
        # SSE.Views.Toolbar.tipTextOrientation から取ります
        # **本家の韓国語は未訳**(英語のまま入っていました)。
        # LibreOffice の公式韓国語(Text orientation)から取りました
        "方向": "텍스트 방향",
        # **セルの書式設定**(2026-08-21)。日本語は Excel の言葉にしたので
        # 本家の日本語(「セルをフォーマットする」)と字面が合いません。
        # 訳は本家の SSE.Views.DocumentHolder.txtCellFormat から取ります
    ...
    "zh": {
        ...
        "プラグインの管理": "管理插件",
        "マクロを書く": "编写宏",
        # **「ふりがな」と同じラベルになっていました**(2026-08-21)。
        # ルビを手で入れるボタンなので、LibreOffice の同じ機能の名前
        # (Asian Phonetic Guide)を使います
        "ルビ": "亚洲文本标音",
        "区切り位置": "分列",
        "印刷レイアウト": "打印布局",
        "図表番号": "题注",
