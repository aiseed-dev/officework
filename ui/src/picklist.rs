...
//! (2026-08-21。記号はセルの並び — `Place::grid`)。
//!
...
pub struct Place {
    ...
    pub width: Width,
    /// **セルの並びで出す**(記号などのパレット。手順4 の最初の一歩)。
    /// 値はセルの一辺(倍率を掛ける前)。`None` なら今までどおり縦の一覧。
    /// セルは見出しの字をそのまま真ん中に置き、絞り込み・色見本・書体の
        ...
        p = p.child(
            div()
                ...
                .child(SharedString::from(if *empty {
                    crate::t!("type_filter_2").to_string()
                } else {
                    text.clone()
                })),
        );
    ...
    if items.is_empty() && filter.is_some() {
        p = p.child(
            div()
                ...
                .text_size(px(s * 12.0))
                .text_color(look.dim)
                .child(crate::t!("not_list_press_enter")),
        );
    }

    // セルの並び(パレット)か、縦の一覧か。同じ器で並べ方だけ変えます
