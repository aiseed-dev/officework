...
//! **組み立ては殻の仕事、何をどの鍵に割り当てるかはこちらの仕事**。
//!
//! Kotlin / Swift の殻もこの表を読む。だから
//! 「鍵の書き方が読めるかどうか」の判定は [`compose_keys`] に**引数で
//! 渡す** — gpui の殻は `gpui::Keystroke::parse` を、他の殻はそれぞれの
...
///
/// `readable` は**鍵の書き方が読めるか**の判定。gpui の殻は
/// `gpui::Keystroke::parse(part).is_ok()` を渡す。2026-08-15 に gpui の
/// 直呼びからここへ出した — この関数を殻から出すため。鍵の書き方の
/// 決まりは殻ごとに違うので、**判定を持たずに受け取るのが正しい**
pub fn compose_keys(
    defaults: &[(&str, &str)],
