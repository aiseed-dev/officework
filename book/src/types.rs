...
impl Cell {
    ...
    /// 見分ける決めは [`crate::is_formula_cell`] の1つだけを見ます —
...
pub struct TextFmt {
    ...
    pub sup: bool,
    pub sub: bool,
    /// **字の大きさ(pt)。** `None` は既定(11pt)です。
    ///
    /// 図をこちらで描くようになって要りました(2026-08-27)。図の題と
...
pub struct Book {
    ...
    /// **ブックの構造を守る。** シートの追加・削除・並べ替え・名前の変更を
    /// 禁じます(xlsx の `workbookProtection` の `lockStructure`)。
    ///
    /// 鍵ではありません — password は書きません。掛けた振りをしないためです
    /// (`read_only_rec` と同じ考えです。2026-08-30 発注者「ブックを保護」)。
