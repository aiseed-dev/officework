...
impl Render for Writer {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // **紙を 1 枚ずつ積む表示では、境の線は引きません。** 紙どうしの
        // 間が空いていて、影も付いているので切れ目は見えます。線は「どの紙も
        // 同じ高さ」として引いていたので、節で紙が変わる文書では紙の途中を
        // 横切っていました(2026-09-21 発注者「フッターの上に横線があるのは
        // おかしくないか」。Word の PDF にもうちの PDF にも線はありません)
        if !self.sheets() {
            let mut pno = 1;
            loop {
