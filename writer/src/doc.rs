        ...
        self.page_tops = if self.sheets() {
            ...
            let papers: Vec<kumihan::PageSetup> = self.page_papers.iter()
                .map(|q| kumihan::PageSetup {
                    ...
                    char_grid: self.pg.char_grid, char_space_pt: self.pg.char_space_pt,
                    top_fixed: self.pg.top_fixed, bottom_fixed: self.pg.bottom_fixed,
                    first_top_mm: self.pg.first_top_mm, first_bottom_mm: self.pg.first_bottom_mm,
                })
                .collect();
