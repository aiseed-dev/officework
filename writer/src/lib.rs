...
pub struct Writer {
    ...
    btn_box: std::rc::Rc<std::cell::RefCell<std::collections::HashMap<&'static str, (f32, f32, f32, f32)>>>,
...
impl Writer {
    ...
    /// 動かず、**配っている officework では控えが1つも取れていません
    /// でした**(実機で確かめた)。
    /// **錠を外す**(2026-08-21)。統合アプリが閉じるときに全部のタブへ
    /// 呼びます。
    ///
    /// *前は単体の `run()` の中でだけ外していました* — officework で普通に
    /// 閉じると `.~lock` が残り、共有フォルダの**他の人は永久に読むだけ**に
    /// なっていました(実機で確かめた)。自分自身の錠は先客と見ない作りなので、
    /// 同じ人には見えず、気づけない形でした。
    pub fn 錠を外す(&mut self) {
        self.release_lock();
    }
