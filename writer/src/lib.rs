...
pub struct Writer {
    ...
    scroll_mm: f32,
    /// **画面に出しているセクション**(用紙の変わり目で区切った束の番号)。
    /// Word の文書は節ごとに用紙を変えられます。Excel のシートと同じで、
    /// 画面はいつも 1 つのセクションだけを見せ、下のタブで選びます
    sect_view: usize,
    ...
    page_tops: Vec<f32>,
    /// **合成後の段落の背景色と囲み**(本文のバイト範囲, 背景色, 囲み)。
    /// 画面の帯はこれで描く。前は合成前の `self.doc` から拾っていたので、
    /// テンプレートが決めた背景(註記の帯・コードの塊)が画面に出なかった
    /// (2026-09-03、塊の見本で見つけた)。組むたびに `lay` が作り直す
    para_deco: Vec<(std::ops::Range<usize>, Option<String>, bool)>,
