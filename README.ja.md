...
| Python(マクロの代わり) | .py をサンドボックスで回すマクロ(d = python-docx の文書)。文書には載せない | plugins の .py — 手続きは @名前、セル関数は =倍(A1) や =PY。ブックには載せない |
...
**VBA 型のマクロはありません。ブックはコードを運びません。** 表は .xlsx、
コードは .py — どちらも別々のファイルです。手続きも自作関数も
`~/.config/office/plugins/` の .py に書き、手続きは `@名前` で実行、
セルからは `=倍(A1)` のような普通の関数呼び出し(名前は日本語でよい)か
`=PY(…)` で使います。受け取ったブックにはコードが入っていないので、
「開く=実行」という攻撃経路は最初から存在しません。
