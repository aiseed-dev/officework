...
- `writer` — **AsciiDoc の視覚エディタ**。原稿(.adoc)を見たまま直し、画面・PDF・HTML・docx に出す。docx は受け渡しの形式で、開いて見る・adoc にすることはできる([手引き](docs/writer-manual.ja.md))
...
```console
$ pip install officework      # エンジン・橋・MCP、そしてアプリ2本
$ officework-calc             # 画面のある表計算が立ち上がる
$ officework-writer
```

wheel は Linux と Windows で約 36MB(mac は2つの CPU 分が入るのでもう少し大きくなります)。**この道がいちばん手間がかかりません** — pip が置いた
...
残っていないこと・**13言語が同じキーを持つこと**を毎回確かめます。
