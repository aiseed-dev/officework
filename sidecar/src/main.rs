...
fn dispatch(
    ...
    match cmd {
        ...
        "read_media" => {
            ...
            match read_media(sess, &s("visualId")) {
                Ok(v) => ok(&rid, v),
                Err(e) => fail(&rid, "io_error", &e),
            }
        }
...
fn attr_of(e: &quick_xml::events::BytesStart<'_>, k: &[u8]) -> Option<String> {
    e.attributes()
        .flatten()
...
fn part_text(z: &mut zip::ZipArchive<impl Read + Seek>, part: &str) -> Option<String> {
...
fn rels_map(z: &mut zip::ZipArchive<impl Read + Seek>, part: &str) -> BTreeMap<String, String> {
...
fn sheet_parts(z: &mut zip::ZipArchive<impl Read + Seek>) -> Vec<String> {
...
fn anchor_value(a: &Corner, b: &Corner) -> Value {
    json!({
...
fn span_to(from: &Corner, ext: (i64, i64), sh: &Sheet) -> Corner {
...
fn media_index(open: &Value) -> BTreeMap<String, (String, String)> {
    open.get("visuals")
...
fn read_media(sess: &Session, visual_id: &str) -> Result<Value, String> {
    let Some((part, mime)) = sess.media.get(visual_id) else {
        return Err(format!("その絵はありません: {visual_id}"));
    };
    let mut z = open_zip(&sess.path)?;
