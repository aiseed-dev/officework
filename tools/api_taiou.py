...
GROUPS = [
    ...
    ("グループ化", ["group", "ungroup"],
     "s.row_dimensions.group(始, 終)", "", "ws.column_dimensions[…].outline_level"),
...
MICHI = {
    ...
    # 書式を外す専用の呼び方はありません(書き方は KAKEBA)。r.clear() は字を消す物です
    "clearstyle": ('', '', ''),
    "merge": ("cell.merge(相手) / s.merge_cells('A1:B2')", 'cell.merge(相手)', "ws.merge_cells('A1:B2')"),
    ...
    "ai-furigana": ('', '', ''),
    "hidenchars": ('', '', ''),
    # ピボットテーブル・数式・データ(2026-08-31)
    "pivot-totals": ('s.add_pivot(…, totals=True)', '', ''),
    "pivot-subtotals": ('s.add_pivot(…, subtotals=True)', '', ''),
    "pivot-showas": ("s.add_pivot(…, agg='sum' / 'mean' / 'count')", '', ''),
    "pivot-refresh": ('', '', ''),
    "pivot-refresh-all": ('', '', ''),
    "pivot-source": ('', '', ''),
    "pivot-fields": ('', '', ''),
    ...
    "watch": ('', '', ''),
    "custom-sort": ('', '', ''),
    "show-details": ('s.row_dimensions.group(始, 終, hidden=False)', '', 'ws.column_dimensions[…].outline_level'),
    "hide-details": ('s.row_dimensions.group(始, 終, hidden=True)', '', 'ws.column_dimensions[…].outline_level'),
    "goal-seek": ('', '', ''),
    "datatable": ('', '', ''),
    "forecast": ('', '', ''),
    "dv-mark": ('', '', ''),
...
TSUKURANAI = {
    ...
    "fn-financial": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "fn-logical": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "fn-text": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "fn-datetime": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "fn-lookup": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "fn-math": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "fn-more": "関数の挿入の小窓を、この族で絞り込みます。文書は変わりません",
    "trace-dep": "参照先を矢印で画面に出します。文書は変わりません",
    "remove-arrows": "画面の矢印を消します。文書は変わりません",
    "watch": "見張る値を画面に並べます。文書は変わりません",
    "pivot-fields": "フィールドの一覧を画面に出します",
    "pivot-select": "画面の選択です",
...
KAKEBA = {
    "clearstyle": "r.bold = None(書式を1つずつ None にする)",
    ...
    "flash-fill": "s['B2'] = s['A2'].value.split()[0](規則を書く)",
    "text-column": "s['B2'], s['C2'] = s['A2'].value.split(',')",
    ...
    "func-list": "~/.config/officework/funcs/ に自分の .py を置く",
...
HOKA = [
    ...
    ("シート見出しの右クリック", "move_left", "b.move_sheet(名前, -1)", "", "wb.move_sheet(名前, -1)"),
    ("シート見出しの右クリック", "move_right", "b.move_sheet(名前, 1)", "", "wb.move_sheet(名前, 1)"),
