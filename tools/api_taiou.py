...
TSUKURANAI = {
    ...
    "draw-select": "手書きの選択です。画面の操作です",
    "pen": "手書きは docx だけの機能で、AsciiDoc に居場所がありません",
    "highlighter": "同じく手書きです",
    "eraser": "同じく手書きです",
    # マクロ(Python を動かす側)
    "py-list": "Python を動かす側です。プログラムからは自分で書けば済みます",
    "py-new": "同じく Python を動かす側です",
    "py-folder": "画面の操作です。プログラムは os が持っています",
    "ribbon-list": "Python を動かす側です",
    "rec-toggle": "画面の操作を記録します。プログラムは自分で書けば済みます",
    "ai-macro": "画面の会話です",
...
HOKA_TSUKURANAI = {
    ...
    "マクロの割り当て": "図形にマクロを結び付けます。プログラムからは直に呼べます",
    "画像として保存(SVG)": "画面の書き出しです",
