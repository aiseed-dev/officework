...
使い方の帳簿は逆で、「できることだけ書く」(.claude/skills/manual/SKILL.md「できることだけ書く」)。
...
SUNSET = [
    ...
    (
        # @取り込み net / @name net の類い。英語の普通の net を巻き込まないよう
        # 「@語 + 空白 + net」だけに絞る
        re.compile(r"@\S+ net\b"),
        re.compile(r"廃止|要らなくなり|無くなった|区別|gone|no longer"),
        "2026-08-09 廃止(@名前 net)。plugins は自分のコードなので網は普通に使える",
    ),
]
