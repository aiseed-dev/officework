...
def ファイルごと旗つき(p: pathlib.Path) -> bool:
...
def 旗の内側(lines: list[str], i: int) -> bool:
    ...
    そうでした)。
    """
    if any(旗の行(l) for l in lines[max(0, i - 6):i]):
        return True
    for j in range(i, -1, -1):
        if re.match(r"\s*(pub(\(\w+\))? )?(async )?fn ", lines[j]):
            return any(旗の行(l) for l in lines[max(0, j - 8):j + 1])
        # モジュールの頭まで来たら、そこを見て終わり
        if re.match(r"\s*(pub )?mod \w+ \{", lines[j]):
            return any(旗の行(l) for l in lines[max(0, j - 4):j + 1])
    return False
