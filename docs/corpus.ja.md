...
1. **中身が正しくなった。** `bojsjfy.xlsx` のシート「47」の `D4` は
   `（２）金融機関 (Financial Institutions)`、`D69` は `調整差額` —
   目録に書いた「正しくはこう」と**一字一句一致**した
   (直す前は `sheet3.xml` の中身を返していた)
2. **取り違えの差が消えた。** `bojsjfy` / `bojsjpre` / `bojsjlong` の3枚で、
   値・式・結合の食い違いが **117 / 117 / 158 行 → 0 行**。
   残る差は `~`(ふりがなの連結=**向こうの欠陥**)と、
   向こうが索引を途中で切っている「使っている範囲」の数だけ。
   最小再現の `lo_manysheets` は ○ になり、`make_manysheets` も
   シートのずれは消えた(残るのは「キャッシュ値の無い式」の決めの件だけ)
3. **往復しても中身が動かない。** 3枚(30/30/38枚)を開いて保存して開き直し、
   全シートの値を突き合わせて**動いたシート 0**

**書き出しにも同じ穴があった。** 本体は並び順に `sheet1..N` と振り直して
書くのに、原本の rels と `[Content_Types]` の宣言をそのまま持ち越していたので、
開き直すと `<sheet>` が別の部品を指す。引き継ぐ印刷設定・図形・シートの rels も
**部品の番号で**数えていた。受入試験 `xlsx::sheet_rid` の
「往復してもシートの中身が動かない」は、直す前は `表2` の中身が
`部品7` に化けて落ちる。

**型紙は手で組んだ**(12枚・`<sheet>` の並びと部品の番号を逆に食い違わせる)。
`make_manysheets` でも再現するが、**受入試験は現物を要らなくしておく** —
corpus は目録から取り直す物なので、試験がそれに依ると CI で回らない。
...
**直した(2026-08-09)。** `CondKind::Formula` を足し、`expression` を読み・
**効かせ**・書き戻す。`make_cond` / `lo_cond` とも報告が消え、両方の書き手から
`Formula("LEN(C2)>3")` として同じに読める。

決めどころは**評価するか、運ぶだけか**。**評価する**ことにした — 縞が出ないなら
読めても仕方がない。式は**範囲の左上を錨にした原文**で持ち、セルごとに左上からの
ずれだけ相対参照をずらして(`model::offset_refs`)その位置で解く。`$` で固定した
側は動かないので、`$D2="済"` の行まるごとの色分けも意図どおりに効く。

**ずらすのは解くときだけで、持っている原文は書き換えない。** 保存はいつも原文を
返すので、**評価が外れてもファイルは減らない**。解けない式・誤りは
「当たり無し」へ倒す(見当違いの色を自信たっぷりに塗るほうが気付けない)。
範囲は 20,000 セルまで — 式は1セルずつ字句解析から解くので、列まるごとを
毎回の描画で回せない。

**ついでに出た1件**: `parse_dxfs` が塗りを `bgColor` からしか読んでおらず、
**openpyxl が書いた条件付き書式の色が全種類で黙って消えていた**
(openpyxl は `patternType="solid"` + `fgColor` に書く。Excel も同じ流儀を使う)。
規則は残って色だけ消えるので気付きにくい。`rgb` を持っているほうを採るよう直した。
...
(設計 docs/sekkei/pyoffice.ja.adoc)。当方の見立ては
**うちに寄せる** — 空欄を返しても呼ぶ側は結局 `recalc_cells` を呼ぶので、
最初から埋まっているほうが往復が減る。

## 第3便: 直しの効きを数で見た(2026-08-09 `main` = `e147bfc`)

シートの取り違え(`127e762`)と名前の定義(`e147bfc`)が着地したので、
corpus 24 枚 + `sample/` 6 枚をサイドカー同士で通し直した。**57,350 セル。**
...
- **シートの取り違え** — `lo_manysheets` が ○。`make_manysheets` に残るのは
  `B2`(`=A2*2`)**1セルだけ**で、しかも中身のずれではなく上の「キャッシュ値の
  無い式」。**`A1` の「これは 表30 です」の類は全部消えた**
- **日銀の 30 枚**(`bojsjfy` / `bojsjpre` / `bojsjlong`)も、残るのは
  `使っている範囲` の 1〜3 行差だけ。**値も式も1つも違わない**
- **名前の定義** — `lo_arrays` が ○。LibreOffice が書いた名前つき範囲が
  式から引けるようになった

### 「使っている範囲」をどうするか(決めが要る)

向こうは `<dimension ref="A1:AL108">` の**宣言**を返す。こちらは**実際に
中身のあるセルの最大**を返す。家計調査で 108 対 106、日銀で 71 対 70。

**`max(宣言, 実際)` に寄せるのが安全。** 宣言だけを信じると、宣言を
当てにしない書き手(古い変換物によくある)で足りなくなる。実際だけを見ると、
**罫線だけ引いた行が画面から消える** — 帳票では見た目が変わる。

### キャッシュ値の無い式をどうするか(決めが要る)

**当方の見立ては「うちに寄せる」。** 空欄を返しても呼ぶ側は結局
`recalc_cells` を呼ぶので、最初から埋まっているほうが往復が減る。
`=A2*2` を空で返して後から埋めるより、開いた時に `400` と言うほうが速い。

## 第4便: `expression` が着地した(2026-08-09)

条件付き書式の `expression` を `main` へ載せて、同じ 30 枚を通し直した。
**57,350 セル。× は 8 枚 → 6 枚。減った 2 枚はちょうど `lo_cond` と
`make_cond`** で、どちらも ○ になった。
...
残る 6 枚は第3便の表のうち**決めが要る 2 種だけ**(使っている範囲 4 枚・
キャッシュ値の無い式 2 枚)。**欠陥として残っているものは無い。**

読みの側も直に確かめた。`lo_cond` / `make_cond` のどちらも
**「読めない種類」の報告が消え**、LibreOffice と openpyxl という別々の書き手から
`Formula("LEN(C2)>3")` として**同じに**読める。

**この 2 枚で `当たり 0 セル` は正しい。** C 列は「品目2」など**3文字**しか
入っていないので `LEN(C2)>3` は偽になる。**0 を見て「効いていない」と早合点しない** —
ずらした式(`LEN(C3)>3`, `LEN(C4)>3`…)と各セルの値まで出して確かめた。
当たる側は試験(`数式で指定した縞模様が往復して効く`)が
`MOD(ROW(),2)=0` で A2/A4 が真・A1/A3 が偽・範囲外は偽まで押さえている。
