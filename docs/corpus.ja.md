...
真偽の属性は**偽なら無いのと同じ**、立っていたら(隠し名前・マクロ)今までどおり
原文で持ち越す。中身を持つ属性(`comment` 等)も、単純と見ると保存で黙って
落ちるので原文へ回す。`lo_arrays` の `式!F7` が `#NAME?` → `15.0` になり、
corpus 24 枚すべてで名前が往復する。試験は
`既定値つきの名前が式から引ける` / `隠し名前は原文のまま持ち越す`。
