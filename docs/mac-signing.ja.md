...
`.p12` の書き出しも base64 も貼り付けも、このスクリプトが繋いでやる。
**秘密は画面に出ない**(`gh` へ直に流す)。

> **`gh`(GitHub CLI)を使っていないなら**、スクリプトは使えない。
...
C のスクリプトも**ファイル名から読み取る**ので、打たなくてよい。
...
> 他人と共有している Mac では、このスクリプトは使わず末尾の「手で入れる」へ。
> 合言葉が一瞬 `ps` に見えるため。
...
GitHub の macOS の機械が、手元と**同じスクリプト**
...
1. **Release を消す** —
   ...
   開き、右上の **ゴミ箱**(Delete)
2. **タグを消す** — Release を消してもタグは残る。普通の git でよい:
...
したか分からない物を作らない**ために、スクリプトはここで止まる。使う方の
SHA-1(行頭の 40 桁)を控えて:

GitHub に作らせているなら Secrets に `MAC_SIGN_IDENTITY` を足す
(C のスクリプトは2枚以上あると聞いてくるので、自分で足す必要は無い)。
手元で回すなら:
...
スクリプトが自分で中間証明書を入れて見直すようにした(2026-08-17)。それでも
...
同梱 Python の `bin/pip3.14` のような shebang のスクリプトも、実行権があれば
codesign は包みの中身と見なす(2026-08-17 に踏んだ)。

スクリプトは Mach-O だけでなく**実行権のあるファイルも署名する**ようにした。
スクリプトには hardened runtime も権利も要らないので素で署名する。

### 公証が通らない

スクリプトが直近の記録を出す。多いのは「署名し漏れた Mach-O がある」で、
...
C のスクリプトが使えない・使いたくないときの道。やることは同じ。
