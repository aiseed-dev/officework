...
_日本語版(secondary): link:from-excel.ja.adoc[from-excel.ja.adoc]_

Started 2026-08-08 (the client: "why not distill Excel's manual into officework's?").
We take *the list of tasks in Excel's official help as our teacher* and write, task by task, "that job you used to do in Excel — here is how you do it here (or why we don't)".
No prose is copied — only the structure and the inventory of tasks are the teacher (the same practice as the ledger that used the vendor as a teacher for behavior).

Second draft (2026-08-08): sixteen sets of tasks were thickened against the real code.
Detailed operations are sent off by link to the link:calc-manual.adoc[calc manual] and the link:python-manual.adoc[Python manual] (not transcribed here).
The inventory of what remains lives in the link:guide-tsukiawase-2.ja.adoc[ledger].

Third draft (2026-08-08): the eighteen areas the second draft listed as "sections still to fill" (cell formatting basics, the body of find & replace, consolidating data, accessibility, and the rest) were filled in with the same pattern (parallel research → adversarial verification).
With that, we have been once around the territory Excel's help covers.
...
Examples: link:../templates/README.adoc[the inquiry ledger] and link:../sample/README.adoc[the order ledger].
...
|LAMBDA |No |Excel puts the expression into a defined name and calls it, but *our names can hold nothing but ranges*, so it is deferred (the reason is recorded on the ledger). A Python function under =PY does the same job (link:python-manual.adoc[the Python manual])
...
|Consolidate by category (matching on the top row and left column labels) |Not yet |Absent. Stack up `=SUMIF(Apr!A2:A50,$A2,Apr!B2:B50)+…`, one term per sheet. When the rows don't line up, or there are many sheets, polars' concat and group\_by are the sure route — and inconsistent labels get cleaned up in the same pass (a candidate for the ledger)
...
|Distinct count |Not yet |The built-in pivot offers five aggregations: sum, average, count, max, min. For now, polars' n\_unique in Data > Python (on the ledger)
...
* *The only thing a workbook may carry is a =PY function.* Procedures run only from .py files you placed in `~/.config/officework/plugins/` yourself — a received file can never become the origin of execution (settled 2026-08-08)
...
In many cases the feature itself is there and only the key is unwired (open on the ledger under "shortcuts and formats").
...
The 18 uncovered areas listed in the second draft were all filled in the third — *that completes one pass over Excel's help territory.* From here, what gets added to this guide is keeping up with newly built features and comparing against whatever Excel adds next.

What remains is not missing sections but *outstanding content*, and it lives in three places:

* *The implementation backlog* = link:guide-tsukiawase-2.ja.adoc[the ledger]. What this draft's comparison turned up (Consolidate, the data model, the Analysis ToolPak function families, FREQUENCY and LINEST, custom lists, form controls, sheet groups, document inspection, creating a workbook from a template, screen reader support, and more) is marked "a backlog candidate" inline in the text, so moving those onto the ledger is the next tidying pass
* *Thin sections* — "Entering data and formatting" is still mostly number formats and merging. The finer points of cell formatting were split out into "Cell formatting basics", so the two should eventually be merged
