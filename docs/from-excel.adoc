...
* VBA is *deliberately absent* — we are not importing the thirty-year-old
  ...
  "open = execute" hole (link:sekkei/python.ja.adoc[design]).
Python takes the job:   link:python-manual.adoc[Python manual]

* *The only thing a workbook may carry is a =PY function.* Procedures run

  only from .py files you placed in `~/.config/officework/plugins/` yourself —   a received file can never become the origin of execution (settled 2026-08-08)
