...
| ✔ print_title_cols | 足す | **列**の繰り返し(**済 2026-08-13**: 模型に畑を足し、Print_Titles の行の部と列の部を別々に解いて往復する(前は列だけの帳票は原文で持ち越すだけだった)。openpyxl と同じ "A:B" の形。適合は両方向。**PDF も列を繰り返す**(済 2026-08-13: 描く側が列を「連番(左端, 本数)」で持っていたのを**列の一覧**に変えた。規則は `paper::grid::band_cols` — 束より左のタイトル列だけを左端に差し込む(束の中や右のものは繰り返さない = 同じ列が1枚に二度出ない)。繰り返す列は**幅の割り付けにも効く**ので、そのぶん本体は狭くなり紙が増える(Excel と同じ)。橋の page_setup にも title_cols を足した。12か月の台帳を実機の calc で PDF にし、4枚とも左端に品名が出ることを目で確かめた)) |
...
| ✔ column_dimensions[…].width / row_dimensions[…].height / hidden | 足す | 列の幅・行の高さ・隠し。**塞いだ穴**(2026-08-13、PDF のタイトル列を実機で試そうとして判明): 模型(col_width / row_height / col_hidden / row_hidden)も xlsx の読み書きも前から動いていたのに、openpyxl 側の口が group だけで、`ws.column_dimensions["A"].width = 20` という**本家で最も使われる書き方**が通らなかった — 帳票の形そのものを Python から作れない。添字で引く窓を足した(書けばその場で載る。本家も同じ)。幅は字数・高さはポイント、指定の無い行列は None(本家と同じ)。行に width・列に height は黙って別の畑へ書かず断る。適合は両方向 |
...
| ずれ | 重さ | どうする |
|---|---|---|
| 指定の無い文字の大きさが往復で焼き付く | 直した(2026-08-13) | `Run.size_pt` を `Option<f32>` にして根治(195箇所・5クレート)。読みは `w:sz` の無い run を `None` のまま持ち、書きは `Some` のときだけ `w:sz` を出す。画面・紙は写す瞬間に文書の既定(docDefaults の `w:sz`、無ければ 10.5)で解く。Python の `size_pt` も None を返す — 本家の `font.size` と同じ約束。検査は test_shiyou.py の「無指定は無指定のまま往復する」 |
