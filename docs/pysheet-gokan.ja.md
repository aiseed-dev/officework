...
| ✔ sections / add_section | 足す | エンジンの模型が1節しか持てない既知の残り(「実物で測った」の表)— 模型を太らせる件と同じ一件(**sections は済 2026-08-13**: 模型は途中の節(段落の sect)+文書末(sect_raw / page)を既に持っていた — 手を張った。紙の大きさ・余白・向きの読み書き(本家の Length で受ける)。**書きは原文の sectPr への属性差し替え**(twips)— ヘッダー参照・段組みなど理解しない設定は崩れない。適合は両方向: 本家の2節(Letter 縦+A4 横)を読み、余白の書きが往復して本家で読める。**add_section は残** — 節を段落の性質で持つ模型で「どの段落で切るか」を決めてから。正直に断る) |
