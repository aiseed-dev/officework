...
| epoch / excel_base_date | 足す | 1904 起点のブック(古い Mac 由来)を 1899-12-30 として読むと日付が4年ずれる — 黙って壊すのと同じ。起点の読みと解釈をエンジンに。**現状(2026-08-13 実測)**: 読みと正直な報告は済 — xlsx/read.rs が workbookPr の date1904 を見て「日付表示は4年ずれます。保存では保たれます」と Report に言う。残るは**解釈の貫通**: 内部は 1899 のまま、通し番号↔暦日の境目(funcs.rs の EXCEL_EPOCH_DAYS 15箇所と fmt.rs の format_value)へ `excel_epoch(date1904)`(1904 は 25569−1462=24107)を通す。funcs::call と format_value の署名に旗が要り、外の呼び口7箇所(calc の表示3・pysheet1 ほか)と pysheet の日付入力(date_serial)も追随 — 評価器の芯に触るので独立の便で |
