...
* 空のセルは *None か ""* で返る(触ったことのないセルは None。
  ...
  "" は**原本に空文字のセルがあった**とき — こちらから `s["A1"] = ""` と  書くのは消すのと同じで、読み直すと None。
どちらも偽なので  `if s["A1"]:` で足りるが、厳密には `s["A1"] in (None, "")` で見る)
...
* まだ何も無い所も `ws.cell(50, 1)` で**参照だけの Cell** が返る
  ...
  (value は None。
書けばその場に入る — openpyxl と同じ感触)

* `insert_rows` / `delete_rows` / `insert_cols` / `delete_cols` は

  `amount=` つき。
`merged_cell_ranges`・`freeze_panes` も通る
...
* openpyxl が読めるかは向こうの試験でも確かめてある — こちらが書いた
  ...
  xlsx を openpyxl がそのまま読む(*こちらが計算した値も* —   openpyxl 自身は式を計算できない)
...
* 返り値: スカラ → そのセルへ / *1次元リスト → 下へ展開* /
  ...
  *2次元リスト → 右下へスピル*。
展開先に他人のデータがあれば  `#SPILL!` で止まる(潰さない)
...
* link:../sample/差し込み.py[sample/差し込み.py] — 見積書に宛名と数量を差し込む。
...
* link:../templates/README.adoc[templates/] — 問い合わせ台帳(`@取り込み` の

  CSV 取り込み・=PY の状態集計)ほか

* link:../sample/README.adoc[sample/注文書.xlsx] — マスタの入れ替え(`@更新`)と

  JSON の送信(`@送信`)

* link:../sample/README.adoc[sample/受注台帳.xlsx] — 取り込みの控え(K2)で
