...
**API と試験は写す・実装は写さない**が方針(docs/sekkei/python.ja.adoc)で、在庫は台帳 link:pysheet-gokan.ja.md[docs/pysheet-gokan.ja.md] — 3ライブラリの中核 324 メンバーを1件ずつ判定してある(できる物・作る物・作らない物とその理由)。
...
*何がどこまであるかは対応表を見てください*(link:pysheet-gokan.ja.md[pysheet-gokan.ja.md] — 324 件に判定と理由)。
...
在庫の全量と判定の理由は台帳(link:pysheet-gokan.ja.md[pysheet-gokan.ja.md])に。
