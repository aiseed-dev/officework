...
. メールと通称を入れ、**ディスクに保存**と**鍵ペア情報を指定**にチェック。

   鍵は *2048 ビット・RSA*

. <https://developer.apple.com/account/resources/certificates/list> で
...
. base64 にして `gh` で GitHub の Secrets へ入れる。
...
* *「macOS だけ組む」に入れる* — 署名を試すだけなら Linux と Windows を
  ...
  組む意味が無い(そのぶん20分ほど短くなる)。
出来た `.dmg` は  走行ページの *Artifacts* から落とす
...
. *Release を消す* —
   ...
   <https://github.com/aiseed-dev/officework/releases> でその Release を   開き、右上の *ゴミ箱*(Delete)

. *タグを消す* — Release を消してもタグは残る。普通の git でよい:

   sh    git push origin :refs/tags/<名前>    

   画面からなら <https://github.com/aiseed-dev/officework/tags> の   その行の *…* → *Delete tag*
...
. *鍵を持っている Mac から `.p12` をもらう*(末尾「手で入れる」の1の手順で書き出した物)。
...
. リポジトリの上のタブで *Settings*

   (**アカウント**の Settings ではない。
リポジトリの名前の下に並ぶ方)

. 左の柱の *Secrets and variables* → *Actions*

   (Codespaces / Dependabot もあるが、*Actions* を選ぶ)

. *Repository secrets* の側の緑の *New repository secret*
...
* 権利(entitlements)は
  ...
  link:../packaging/macos/entitlements.plist[packaging/macos/entitlements.plist]   の**2つだけ**。
`pip install` した拡張を読めるようにする物と、ctypes の物。
App Sandbox は入れない(直配布では要らず、入れると  `~/.config/officework/` が読めなくなる)

== まだしていないこと

* *Intel(x86\_64)の .dmg は出していない。* `make-macos.sh` は**走っている*

  Mac の的**で組む(Apple Silicon なら arm64)。
両方出すなら2台で回すか、  universal に組む工事が要る*
