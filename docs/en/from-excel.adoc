...
|Documents in MS Mincho / MS Gothic come out with the wrong widths |Different |On a Mac with Microsoft Office installed nothing needs to be added: the MS Mincho, MS Gothic, Yu Mincho, Yu Gothic and Meiryo fonts bundled with Office are used as they are. On a machine without the Windows fonts (MS Mincho, MS Gothic, MS PMincho, MS PGothic), install the free Ume fonts (Ume Mincho, Ume Gothic, Ume P Mincho, Ume P Gothic), which have the same advance widths. Half-width digits and spaces then measure as in Excel, so column widths and table rows of old documents keep their shape. Put the ttf files in `~/Library/Fonts` on a Mac or `~/.fonts` on Linux. Without them, text is drawn in a system font such as Hiragino while half-width characters are still advanced by 0.5em as MS Mincho does, so widths and line breaks match even though the typeface differs
...
|Entering a formula |Same |403 functions are implemented (see the table below)
