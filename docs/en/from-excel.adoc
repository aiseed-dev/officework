...
The finer points of cell formatting (borders, fills, text alignment, wrapping, vertical text and so on) are still thinly compared against Excel's help — see "Sections still to fill" at the end.
...
The rule types we support round-trip through xlsx; the ones we cannot read are *reported when the file opens, then dropped* — never silently.
