...
**Automating Office work with Python — that is what this software is for. **The engine installs on its own (`pip install officework`): it reads and writes xlsx and docx without breaking their formatting, in the idiom of openpyxl, python-docx and xlwings.
On top of it sit *Word and Excel that run on your machine* — where you watch the result, finish it by hand, and drive a live workbook from Python.

The second purpose points the other way — *using Python from Office*: a cell formula like `=double(A1)` calls your own Python function and the result spills, and the chart and pivot buttons are backed by matplotlib and polars.
This is what replaces VBA:

* `writer` — *a visual editor for AsciiDoc*: edit your source (.adoc) as it looks, and put it out as screen, PDF, HTML or docx. docx is the delivery format — you can open one and turn it into adoc (link:docs/writer-manual.adoc[manual])
* `calc` — opens, edits, and saves xlsx. Calculates formulas (link:docs/calc-manual.adoc[manual])
...
|Drawing (pen, highlighter, eraser) |○ (saved as SVG pictures; also shapes in docx) |—
...
|Protection, encryption, signing |○ (it *opens* an encrypted docx; applying encryption lives in calc) |○ (read-only, AES, Ed25519 side-file signature)
...
*There are no VBA-style macros, and workbooks never carry code.* Sheets live in .xlsx, code lives in .py — separate files.
...
See link:docs/engine.md[the engine page] for the recipe and for exactly what is and is not replaced.
...
The reference for programmers is the link:docs/python-manual.adoc[Python manual] — ranges vs. arrays (`values()` and per-cell writes), 2-D lists and spilling with `=PY`, the `d` binding in writer macros, and what the sandbox allows.
...
There is also a settings page: File > Advanced settings shows the current language, font, proofreading endpoint, and Python path, and cycles through the languages (applies on next start).
...
Design decisions are recorded in `SEKKEI.adoc` (Japanese), history and open items in `HIKITSUGI.md` (Japanese), and there are ready-to-open samples in `sample/`.
