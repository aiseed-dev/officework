...
* `writer` — *a visual editor for AsciiDoc*: edit your source (.adoc) as it looks, and put it out as screen, PDF, HTML or docx. docx is the delivery format — you can open one and turn it into adoc (link:docs/writer-manual.md[manual])
* `calc` — opens, edits, and saves xlsx. Calculates formulas (link:docs/calc-manual.md[manual])
...
See the link:docs/calc-manual.md[calc manual].
...
The reference for programmers is the link:docs/python-manual.md[Python manual] — ranges vs. arrays (`values()` and per-cell writes), 2-D lists and spilling with `=PY`, the `d` binding in writer macros, and what the sandbox allows.
