...
check((h, w) == (4, 4), "広さ: {} 行 × {} 列".format(h, w))
check([ws.cell(1, 5 + c).value for c in range(4)] == ["支店", "4月", "5月", "総計"],
      "見出し")
check(ws["F3"].value == 1500, "東京の4月: {}".format(ws["F3"].value))
...
check(r["H4"].value == 4500, "保存して開き直した総計: {}".format(r["H4"].value))
