...
pub fn handle(h: &mut impl Host, line: &str) -> String {
    ...
    match cmd.as_str() {
        ...
        "layout" => {
            ...
            let sh = &h.book().sheets[si];
            // The column's width in points, from the sheet's millimetres and
            // the workbook's conversion (the same the screen and the print use)
            let basis = h.book().col_basis;
            let col_pt = |c: u32| -> f64 { f64::from(sh.col_haba_mm(c, &basis)) * 72.0 / 25.4 };
        ...
        "col_width" | "row_height" => {
            ...
            match o.num("value") {
                Some(v) => {
                    ...
                    let sh = &mut h.book_mut().sheets[si];
                    if cols {
                        // The command speaks Excel's digits, as the xlsx does
                        for c in a.col..=b.col {
                            sh.set_col_xlsx(c, v as f32, &basis);
                        }
                    } else {
                        for r in a.row..=b.row {
                ...
                None => {
                    ...
                    let vals: Vec<Option<f32>> = if cols {
                        // Answered in Excel's digits, the unit the command takes
                        (a.col..=b.col)
                            .map(|c| sh.col_xlsx_to_write(c, &basis).or(sh.default_col_xlsx_to_write(&basis)))
                            .collect()
                    } else {
                        (a.row..=b.row)
