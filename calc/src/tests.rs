...
//! # 言語を決めてから回してください
//!
//! ここの試験は**画面の日本語の文言を直に見ます**(「断り方が伝わらない」
//! の形)。言語を設定していない機械では英語の文言が返って落ちます。
//!
//! ```text
//! OFFICE_LANG=ja cargo test -p calc
//! ```
//!
//! 前は要りませんでした。既定が ja の決め打ちで、機械の設定を見ていな
//! かったためです。2026-08-30 に設定と OS を見るようにし、どれも無ければ
//! en にしました。CI も同じ環境変数を渡しています。
...
mod numfmt_tests {
    ...
    fn number_formats_apply_from_the_list_and_from_a_typed_code(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("percentage_12_34", cx);
            assert_eq!(
            ...
            this.run_cmd("format", cx);
            this.apply_pick("other_type_format_code", cx);
...
mod sort_tests {
    ...
    fn when_data_continues_beside_the_selection_it_asks_to_extend(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("sort_selection_only_fall", cx);
            assert_eq!(
            ...
            this.apply_pick("expand_selection_neighbouring_columns", cx);
...
mod pivot_tests {
    ...
    fn pivot_rows_columns_and_values_are_chosen_by_clicking_the_list(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("choose_fields_myself_rows", cx);
            assert_eq!(this.pick_kind, "pivot-rows-pick", "カーソルだけで行の一覧が開かない");
            // 見出しを選ばず決定 → 言い返されて一覧のまま
            this.apply_pick("next_choose_columns", cx);
            ...
            this.apply_pick("done_columns_optional", cx);
    ...
    fn the_border_panel_takes_repeated_clicks_and_applies_a_table_shape_in_one(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_borders("all_borders_grid");
            ...
            this.apply_borders("bottom_border");
            let _ = cx;
...
mod recalc_tests {
    ...
    fn freezing_is_chosen_from_a_preset_list(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("freeze_top_row", cx);
            assert_eq!(this.frozen, Some(Pos::new(1, 0)), "最上行が固定されない");
            this.run_cmd("freeze", cx);
            this.apply_pick("freeze_first_column", cx);
            assert_eq!(this.frozen, Some(Pos::new(0, 1)), "最初の列が固定されない");
            this.run_cmd("freeze", cx);
            this.apply_pick("freeze_cursor_rows_above", cx);
            ...
            this.run_cmd("freeze", cx);
            this.apply_pick("shadow_frozen_edge", cx);
    ...
    fn the_rule_manager_can_select_and_delete_a_rule(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("delete_rule", cx);
    ...
    fn the_name_manager_can_move_retype_and_delete(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.run_cmd("defname", cx);
            this.apply_pick("name:単価表", cx);
            this.apply_pick("delete_name", cx);
    ...
    fn headers_and_footers_can_be_set_and_cleared_from_the_panel(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("header_centre", cx);
            ...
            this.run_cmd("edit-header", cx);
            {
                ...
                assert!(
                    items.iter().any(|(k, l)| k == "header_centre" && l == "ヘッダー中: 月次売上"),
                    "{items:?}"
                );
            }
            this.apply_pick("footer_right", cx); // 鍵は欄の名前(値は見出しの側)
            this.prompt = Some(("hf-edit", Editor::new("&P / &N")));
            this.finish_prompt(cx);
            assert_eq!(this.sheet().footer.as_deref(), Some("&R&P / &N"));
            // 全部消す
            this.run_cmd("edit-header", cx);
            this.apply_pick("clear_all", cx);
    ...
    fn more_colors_and_a_typed_text_angle(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("other_type_rrggbb", cx);
    ...
    fn borders_apply_with_the_pens_style_and_color(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("medium_solid", cx);
    ...
    fn merging_applies_without_asking_and_drops_all_but_the_top_left(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("merge_centre", cx);
            ...
            this.sync_input();
            this.run_cmd("merge", cx);
            this.apply_pick("merge_across_row_row", cx);
    ...
    fn a_damaged_workbook_opens_but_refuses_to_be_overwritten(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("salvage_what_can_read", cx);
            // 読めた部品だけで開けている
            assert_eq!(
    ...
    fn a_scenario_stores_only_values_and_writes_them_back(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("new_scenario_current_values", cx);
    ...
    fn the_five_ways_to_change_case() {
        ...
        assert_eq!(change_case(t, "sentence_case"), "Hello world こんにちは 3rd");
        assert_eq!(change_case(t, "capitalise_each_word"), "Hello World こんにちは 3rd");
        assert_eq!(
            change_case(t, "toggle_case"),
...
mod pivot_e2e_tests {
    ...
    async fn the_pivot_works_end_to_end_from_insert_to_close(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            this.apply_pick("apply_filter", cx);
        });
        cx.executor().advance_clock(std::time::Duration::from_secs(30));
        cx.run_until_parked();
        c.update(cx, |this, _| {
...
mod tab_zoom_tests {
    ...
    fn the_tab_menu_wording_follows_the_current_protection_state(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            assert!(items.iter().any(|(k, _)| k == "protect_sheet"), "{items:?}");
            assert!(!items.iter().any(|(k, _)| k == "unprotect_sheet"));

            // 押すと掛かる
            this.apply_pick("protect_sheet", cx);
            ...
            assert!(items.iter().any(|(k, _)| k == "unprotect_sheet"), "{items:?}");
            this.apply_pick("unprotect_sheet", cx);
...
mod symbol_watch_tests {
    ...
    fn symbols_are_chosen_by_set_then_character_with_recents_first(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, cx| {
            ...
            assert!(items.iter().any(|(k, _)| k == "symbols:common_business_forms"), "組が出ていない");
            assert!(items.iter().any(|(k, _)| k.starts_with("type_unicode")), "16進の口が無い");
...
mod csv_out_tests {
    ...
    fn csv_writes_in_shift_jis_and_counts_the_lost_characters(cx: &mut gpui::TestAppContext) {
        ...
        c.update(cx, |this, _| {
            ...
            this.csv_kind = "shift_jis_cp932_comma";
...
mod datefmt_tests {
    ...
    fn japanese_shows_japanese_dates() {
        ...
        assert!(label("weekday_only").ends_with("木曜日"), "{}", label("weekday_only"));
    }
