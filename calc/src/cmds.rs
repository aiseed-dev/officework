...
fn py_cell(c: Option<&book::Cell>) -> String {
    ...
    match &c.value {
        ...
        book::Value::Bool(b) => py_bool(*b).into(),
...
impl Calc {
            ...
            for c in a.col..=b.col {
                ...
                for r in b.row + 1..=to.row {
                    ...
                    if let Some((last, step)) = series {
                        ...
                        cell.formula =
                            Some(book::offset_refs(f, (r - src_r) as i64, 0));
                    }
                    sh.set(p, cell);
                    n += 1;
                }
            ...
            for r in a.row..=b.row {
                ...
                for c in b.col + 1..=to.col {
                    ...
                    if let Some((last, step)) = series {
                        ...
                        cell.formula =
                            Some(book::offset_refs(f, 0, (c - src_c) as i64));
                    }
                    sh.set(p, cell);
                    n += 1;
                }
    ...
    pub(crate) fn fill_handle_auto(&mut self) {
        ...
        match target {
            ...
            None => {
                self.status =
                    ui::t!("no_neighbouring_column_take").into();
            }
        }
    }
    ...
    pub(crate) fn make_table(&mut self, style: crate::util::TableStyle, label: Option<&str>) {
        ...
        self.status = match label {
            Some(l) => ui::tf!(
                "turned_into_table_can",
                a.a1(),
                b.a1(),
                l
            ),
            None => ui::tf!(
                "turned_into_table_header",
                a.a1(),
                b.a1()
            ),
        }
    ...
    pub(crate) fn rec_fmt_diff(&self, before: &book::CellFormat) -> (Vec<String>, bool) {
    ...
    pub(crate) fn rec_value_diff(&self, before: &book::Sheet) -> Option<String> {
    ...
    pub(crate) fn sheet_changed(&self, before: &book::Sheet) -> bool {
        let a = self.sheet();
    ...
    pub(crate) fn rec_page_diff(&self, before: &book::Sheet) -> Option<String> {
        let a = self.sheet();
    ...
    fn run_cmd_inner(&mut self, id: &str, cx: &mut Context<Self>) {
        ...
        if Self::PIVOT_LOCKED.contains(&id) && self.pivot_at(self.cursor).is_some() {
            self.status =
                ui::t!("not_available_pivot_table").into();
            cx.notify();
            return;
        }
        match id {
            ...
            "copystyle" => {
                ...
                let f = self
                    ...
                    .unwrap_or_default();
                self.brush = Some(f);
                self.status = ui::t!("format_picked_up_painted").into();
            }
            ...
            // 縦の揃えと折り返し
            "top" => self.fmt(|f| f.valign = book::VAlign::Top),
            "middle" => self.fmt(|f| f.valign = book::VAlign::Middle),
            "bottom" => self.fmt(|f| f.valign = book::VAlign::Bottom),
            ...
            "fill-num" => {
                ...
                if a.row == b.row {
                    ...
                    if a.row == 0 {
                        self.status = ui::t!("row_1_no_row").into();
                    } else {
                        self.commit();
                        self.checkpoint();
                        let sh = &mut self.book.sheets[self.active];
                        ...
                        for c in a.col..=b.col {
                            ...
                            if let Some(f) = &src.formula {
                                cell.formula = Some(book::offset_refs(f, 1, 0));
                            }
                    ...
                    for c in a.col..=b.col {
                        ...
                        for r in a.row + 1..=b.row {
                            ...
                            if let Some(f) = src.as_ref().and_then(|s| s.formula.as_ref()) {
                                cell.formula =
                                    Some(book::offset_refs(f, (r - a.row) as i64, 0));
                            }
                            sh.set(p, cell);
                            n += 1;
                        }
            ...
            "fill-right" => {
                ...
                if a.col == b.col {
                    ...
                    if a.col == 0 {
                        self.status = ui::t!("column_no_column_left").into();
                    } else {
                        self.commit();
                        self.checkpoint();
                        let sh = &mut self.book.sheets[self.active];
                        ...
                        for r in a.row..=b.row {
                            ...
                            let mut cell = src.clone();
                            if let Some(f) = &src.formula {
                                cell.formula = Some(book::offset_refs(f, 0, 1));
                            }
                    ...
                    for r in a.row..=b.row {
                        ...
                        for c in a.col + 1..=b.col {
                            ...
                            if let Some(f) = src.as_ref().and_then(|s| s.formula.as_ref()) {
                                cell.formula =
                                    Some(book::offset_refs(f, 0, (c - a.col) as i64));
                            }
                            sh.set(p, cell);
                            n += 1;
                        }
            ...
            "merge" => {
                ...
                if self.anchor.is_none() {
                    ...
                    self.pick = Some((
                        menu(&[
                            ui::item!("merge_centre"),
                            ui::item!("merge_across_row_row"),
                            ui::item!("merge_cells_leave_alignment"),
                            ui::item!("unmerge"),
                        ]),
                        at,
                    ));
            ...
            "setfilter" => {
                if self.auto_filter.is_some() {
                    ...
                    self.status = ui::t!("filter_range_removed").into();
                } else {
                    let (a, b) = if self.anchor.is_some() {
                        ...
                        if rows < 2 || cols == 0 {
                            self.status = ui::t!("no_table_filter_needs").into();
                            return;
                        }
            ...
            "clear-filter" => {
                ...
                if self.auto_filter.is_none() {
                    self.status = ui::t!("no_filter_applied").into();
                    return;
                }
                self.auto_filter = None;
                self.filter_panel = None;
            ...
            "pageorient" => {
                ...
                self.dirty = true;
                self.status = ui::tf!(
                    "page_orientation_applies_pdf",
                    if landscape { ui::t!("landscape_2") } else { ui::t!("portrait") }
                )
                .into();
            }
            "pagesize" => {
                ...
                self.status = ui::tf!("paper_b_sizes_jis", name).into();
            }
            "pagemargins" => {
                self.commit();
                self.checkpoint();
                ...
                sh.margins_mm = next;
                self.dirty = true;
                self.status = ui::tf!("print_margins", label).into();
            }
            "printarea" => {
                self.commit();
                if self.anchor.is_some() {
                    ...
                    self.dirty = true;
                    self.status = ui::t!("print_area_cleared_everything").into();
                } else {
                    self.status =
                        ui::t!("select_print_area_shift").into();
                }
            }
            ...
            "format" => {
                ...
                let mut items: Vec<(String, String)> = fmts
                    ...
                    .collect();
                items.extend(menu(&[ui::item!("other_type_format_code")]));
                self.status = match (cur_row, &cur) {
                    (Some((_, l)), _) => ui::tf!("current_format", l).into(),
                    (None, Some(code)) => ui::tf!("current_format_custom_code", code).into(),
                    (None, None) => ui::t!("current_format_general").into(),
                };
            ...
            "fontname" => {
                ...
                if vals.is_empty() {
                    self.status = ui::t!("no_font_found_can").into();
                } else {
                    let at = self.pop_anchor();
            ...
            "trace-prec" => {
                ...
                if deps.is_empty() {
                    ...
                    self.status = ui::tf!(
                        "highlighted_precedent_cells_clear",
                        self.cursor.a1(),
                        deps.len()
                    )
                    .into();
            ...
            "trace-dep" => {
                ...
                if dependents.is_empty() {
                    self.status = ui::tf!("no_formula_refers", me.a1()).into();
                } else {
                    self.status = ui::tf!(
                        "highlighted_dependent_cells_clear",
                        me.a1(),
                        dependents.len()
                    )
                    .into();
            ...
            "remove-arrows" => {
                if self.trace.is_empty() {
                    self.status = ui::t!("no_trace_arrows_showing").into();
                    return;
                }
                self.trace.clear();
                ...
                if let Some((a, b)) = picked {
                    if b.row <= a.row {
                        ...
                        if self.pivot_suggest_pick() {
                            self.status = ui::t!("choose_suggested_layout_can").into();
                        } else {
                            self.status = ui::t!("click_headers_lay_out").into();
            ...
            "prot-doc" => {
                ...
                if self.sheet().protected {
                    ...
                    self.dirty = true;
                    self.status = ui::tf!(
                        "protected_sheet_only_unlocked_cells",
                        name
                    )
                    .into();
                }
            ...
            "cell-lock" => {
                ...
                self.status = if all_locked {
                    ui::t!("unlocked_cell_stays_writable").into()
                } else {
                    ui::t!("locked_not_writable_while").into()
                };
            }
            ...
            "cell-hide-formula" => {
                ...
                self.status = if all_shown {
                    ...
                    if self.sheet().protected {
                        ui::t!("formula_hidden_not_show").into()
                    } else {
                        ui::t!("marked_hide_formula_takes").into()
                    }
                } else {
                    ui::t!("formula_no_longer_hidden").into()
                };
            }
            ...
            "prot-allow" => {
                ...
                self.pick_note = Some(
                    ui::t!("allowed_while_protected_click").into(),
                );
                self.pick = Some((items, at));
            }
            ...
            "prot-encrypt" => {
                ...
                self.status = if self.encrypt_pw.is_some() {
                    ui::t!("encryption_type_new_password").into()
                } else {
                    ui::t!("encrypt_type_password_press").into()
                };
            }
                ...
                let items = vec![
                    (
                        ...
                        if self.comment_list.is_some() {
                            ui::t!("close_comment_list").to_string()
                        } else {
                            ui::t!("show_comment_list").to_string()
                        },
                    ),
                    (
                        "tips".to_string(),
                        ...
                        if self.show_comments {
                            ...
                            ui::t!("show_notes_cells").to_string()
                        },
                    ),
                ];
                let at = self.pop_anchor();
            ...
            "co-delcomment" => {
                ...
                let items = vec![
                    (
                        "here".to_string(),
                        ui::tf!("comment_cell", self.cursor.a1()),
                    ),
                    ("mine".to_string(), ui::t!("my_comments").to_string()),
                    (
                        "all".to_string(),
                        ui::tf!("all_comments_whole_workbook", n_all),
                    ),
                ];
                let at = self.pop_anchor();
                self.pick_note =
                    Some(ui::t!("pick_how_far_delete").into());
            ...
            "co-history" => {
                if self.path.is_none() {
                    ...
                    if v.is_empty() {
                        ...
                        self.status =
                            ui::t!("version_history_pick_one").into();
                    }
                }
            }
            ...
            "co-chat" => match self.chat_path() {
                None => {
                    self.status =
                        ui::t!("not_file_yet_save_get").into();
                }
                Some(cp) => {
                    ...
                    self.status = if tail.is_empty() {
                        ui::t!("no_messages_yet_type").into()
                    } else {
                        format!("言伝: {tail}").into()
                    };
            ...
            "rec-toggle" => {
                ...
                match std::fs::write(&path, script) {
                    ...
                    Err(e) => self.status = ui::tf!("cannot_write_recording", e).into(),
                }
            }
            ...
            "py-new" => {
                ...
                if let Err(e) = std::fs::write(&path, ui::pyedit::skeleton(&format!("新しい道具{n}"))) {
                    self.status = ui::tf!("cannot_create", e).into();
                    return;
                }
                self.status = match ui::open_for_edit(&path.display().to_string()) {
                    Ok(tool) => ui::tf!(
                        "created_opened_save_can",
                        path.file_name().unwrap_or_default().to_string_lossy(), tool
                    )
                    .into(),
                    Err(e) => ui::tf!("created_but_cannot_open",
            ...
            "func-list" => {
                ...
                self.pick_note =
                    Some(ui::t!("can_write_name_cell").into());
                self.pick = Some((
                    fs.iter()
                        .map(|(m, d)| {
            ...
            "ribbon-list" => {
                ...
                self.pick_paths = decls
                    ...
                    .collect();
                items.push((MIHON.into(), ui::t!("create_example").to_string()));
                for (n, _) in pyrun::BUNDLED {
                    items.push((
                        format!("{BUNDLED_MARK}{n}"),
                        ui::tf!("built_read_only", *n).to_string(),
                    ));
                }
                self.pick_kind = "ribbon-macro";
                self.pick_note = Some(
                    ui::t!("py_declares_itself_becomes").into(),
                );
                self.pick = Some((items, at));
            }
            // 置いてある .py の一覧。**選ぶと走る。**
            ...
            "py-list" => {
                ...
                if plugs.is_empty() {
                    self.status =
                        ui::tf!("no_py_files_plugins", dir.display().to_string()).into();
                    return;
                }
                let at = self.pop_anchor();
                ...
                self.pick_note =
                    Some(ui::t!("picking_one_runs_edit").into());
                self.pick = Some((
                    plugs
                        .iter()
                        .map(|(m, d)| {
            ...
            "inscheckbox" => {
                ...
                if empties.is_empty() && bools == 0 {
                    self.status =
                        ui::t!("pick_empty_cell_occupied").into();
                } else {
                    if !empties.is_empty() {
                        self.checkpoint();
                        for p in &empties {
                    ...
                    self.status = ui::tf!(
                        "placed_checkboxes_toggle_space",
                        empties.len(),
                        skip_note
                    )
                    .into();
            ...
            "insslicer" => {
                {
                    ...
                    if rows < 2 {
                        self.status =
                            ui::t!("pick_column_slicer_data").into();
                    } else {
                        ...
                        let at = self.pop_anchor();
                        self.pick_note =
                            Some(ui::t!("pick_columns_slicers_many").into());
            ...
            "insequation" => {
                ...
                self.status =
                    ui::t!("equation_tex_notation_e").into();
            }
            ...
            "inssmartart" => {
                ...
                self.status =
                    ui::t!("smartart_pick_category_layout").into();
            }
            ...
            "solver" => {
                if self.solver.take().is_none() {
                    ...
                    self.status =
                        ui::t!("solver_click_field_type").into();
                }
            }
            // 下付き(vertAlign subscript)。上付きは本家 calc にも無い
            "subscript" => {
                self.fmt(|f| f.subscript = !f.subscript);
                self.status = ui::t!("subscript_toggled").into();
            }
            // 両端揃え(セルの横揃え。折り返した行を左右に伸ばす)
            "align-just" => {
                self.fmt(|f| {
                    f.align = if f.align == book::HAlign::Justify {
                        book::HAlign::General
                    } else {
                        book::HAlign::Justify
                    };
                ...
                self.status = ui::t!("justified_wrap_enabled_too").into();
            }
            ...
            "text-orient" => {
                ...
                let items: Vec<(String, String)> = [
                    (ui::item!("no_rotation"), 0i32),
                    (ui::item!("rotate_up_45"), 45),
                    (ui::item!("rotate_down_45"), 135),
                    (ui::item!("rotate_up_90"), 90),
                    (ui::item!("rotate_down_90"), 180),
                    (ui::item!("vertical_stack_one_character"), 255),
                ]
                .iter()
                .map(|((k, l), deg)| {
                    (
                ...
                .chain(menu(&[ui::item!("other_type_angle")]))
                .collect();
                self.pick_note = Some(ui::t!("text_orientation_xlsx_convention").into());
            ...
            "calc-mode" => {
                ...
                self.status = if self.auto_calc {
                    ui::t!("calculation_automatic_always_immediate").into()
                } else {
                    ui::t!("calculation_manual_f9_recalculates").into()
                };
            }
            ...
            "ref-style" => {
                ...
                self.status = if self.book.r1c1 {
                    ui::t!("reference_style_r1c1_r").into()
                } else {
                    ui::t!("reference_style_a1_usual").into()
                };
            }
            ...
            "cell-styles" => {
                ...
                self.status = ui::t!("cell_styles_pick_one").into();
            }
            ...
            "sheet-view" => {
                ...
                if hidden.is_empty() {
                    ...
                    if self.book.sheets.iter().filter(|s| !s.hidden).count() <= 1 {
                        self.status = ui::t!("last_sheet_cant_hidden").into();
                    } else {
            ...
            "watch" => {
                ...
                if n == 0 && !self.watch.is_empty() {
                    self.watch.clear();
                    self.status = ui::t!("watch_list_cleared").into();
                } else {
                    self.status = ui::tf!(
                        "watching_cells_values_show",
                        n
                    )
                    .into();
                }
            ...
            "pen" | "highlighter" | "eraser" => {
                ...
                self.status = match self.tool {
                    Some(0) => ui::t!("pen_drag_over_grid").into(),
                    Some(1) => ui::t!("highlighter_drag_mark_light_band").into(),
                    Some(2) => ui::t!("eraser_trace_line_remove").into(),
                    _ => ui::t!("back_cell_operations").into(),
                };
            }
            ...
            "colorschemas" => {
                ...
                let items: Vec<(String, String)> = book::theme::SCHEMES
                    .iter()
                    .map(|(n, _)| (n.to_string(), crate::util::color_scheme_label(n)))
                    .collect();
                self.pick = Some((items, at));
                self.status = ui::t!("colour_scheme_pick_one").into();
            }
            ...
            "td-resize" => {
                ...
                match self.sheet().tables.iter().position(|t| t.contains(p)) {
                    None => self.status = ui::t!("put_cursor_inside_table_2").into(),
                    Some(i) => {
                        let t = &self.sheet().tables[i];
                        let init = format!("{}:{}", t.a.a1(), t.b.a1());
                        self.status = ui::tf!("new_range_table", t.name).into();
            ...
            "rtl-sheet" => {
                ...
                self.dirty = true;
                self.status = if on {
                    ui::t!("right_left_layout_columns").into()
                } else {
                    ui::t!("back_left_right").into()
                };
            }
            ...
            "formula-bar" => {
                ...
                self.status = if self.show_formula_bar {
                    ui::t!("formula_bar_shown").into()
                } else {
                    ui::t!("formula_bar_hidden_view").into()
                };
            }
            "show-headings" => {
                self.show_headers = !self.show_headers;
                self.status = if self.show_headers {
                    ui::t!("headings_shown").into()
                } else {
                    ui::t!("headings_hidden_column_width").into()
                };
            }
            "show-zeros" => {
                self.show_zeros = !self.show_zeros;
                self.status = if self.show_zeros {
                    ui::t!("zeros_shown").into()
                } else {
                    ui::t!("zeros_hidden_display_only").into()
                };
            }
            ...
            "subtotal" => {
                self.commit();
                if self.anchor.is_none() {
                    self.status = ui::t!("select_table_range_row").into();
                } else {
                    let (a, b) = self.sel_rect();
                    if b.row <= a.row {
                        self.status = ui::t!("data_rows_needed_below").into();
            ...
            "group" | "ungroup" => {
                ...
                if self.anchor.is_none() {
                    self.status =
                        ui::t!("select_rows_columns_group").into();
                } else {
                    let (a, b) = self.sel_rect();
            ...
            "hide-details" | "show-details" => {
                ...
                if on_cols {
                    ...
                    if targets.is_empty() {
                        self.status =
                            ui::t!("no_grouped_columns_selection").into();
                    } else {
                        self.checkpoint();
                        let sh = self.sheet_mut();
                        for c in &targets {
                    ...
                    let (r0, r1) = if self.anchor.is_some() {
                        ...
                        if !sh.row_outline.contains_key(&at) {
                            self.status = ui::t!("press_grouped_row_data").into();
                            cx.notify();
                            return;
                        }
                        let mut lo = at;
                    ...
                    if targets.is_empty() {
                        self.status =
                            ui::t!("no_grouped_rows_selection").into();
                    } else {
                        self.checkpoint();
                        let sh = self.sheet_mut();
                        for r in &targets {
            ...
            "pivot-source" => {
                ...
                match self.pivot_at(self.cursor) {
                    ...
                    None => {
                        self.status =
                            ui::t!("put_cursor_pivot_want").into();
                    }
                }
            }
            "pivot-refresh" => {
                ...
                match self.pivot_at(self.cursor) {
                    ...
                    None => {
                        self.status =
                            ui::t!("put_cursor_pivot_refresh").into();
                    }
                }
            }
            "pivot-refresh-all" => {
                ...
                if n == 0 {
                    self.status = ui::t!("workbook_no_pivots").into();
                } else {
                    for i in 0..n {
                        let d = self.book.pivots[i].clone();
                        self.spawn_pivot(d, Some(i), cx);
                    }
                    self.status = ui::tf!("refreshing_pivot_tables", n).into();
                }
            }
            ...
            "pivot-showas" => {
                ...
                match self.pivot_at(self.cursor) {
                    ...
                    Some(i) => {
                        ...
                        let at = self.pop_anchor();
                        let items: Vec<(String, String)> = [
                            ui::item!("no_calculation"),
                            ui::item!("total"),
                            ui::item!("running_total"),
                            ui::item!("difference"),
                        ]
                        .iter()
                        .map(|(k, l)| {
                            let key = if *k == "no_calculation" { "" } else { *k };
                            // ✓ は見出しにだけ(鍵はそのまま照合に使う)
                            (
                                k.to_string(),
            ...
            "pivot-style" => {
                ...
                match self.pivot_at(self.cursor) {
                    ...
                    Some(i) => {
                        ...
                        .iter()
                        .map(|(k, l)| {
                            let key = if *k == "blue_default" { "" } else { *k };
                            (
                                k.to_string(),
                                if key == cur { format!("✓ {l}") } else { l.to_string() },
                            )
                        ...
                        .collect();
                        self.pick_note =
                            Some(ui::t!("pivot_style_choosing_reapplies").into());
            ...
            "edit-header" | "editheader" => {
                ...
                let at = self.pop_anchor();
                let (hl, hc, hr) =
                    book::hf_split(self.sheet().header.as_deref().unwrap_or(""));
                let (fl, fc, fr) =
                    book::hf_split(self.sheet().footer.as_deref().unwrap_or(""));
                ...
                let mut items: Vec<(String, String)> = vec![
                    show(ui::item!("header_left"), &hl),
                    show(ui::item!("header_centre"), &hc),
                    show(ui::item!("header_right"), &hr),
                    show(ui::item!("footer_left"), &fl),
                    show(ui::item!("footer_centre"), &fc),
                    show(ui::item!("footer_right"), &fr),
                ];
                items.extend(menu(&[ui::item!("clear_all")]));
                self.pick_note = Some(
                    ui::t!("header_footer_appears_print").into(),
                );
            ...
            "pivot-select" => {
                match self.pivot_at(self.cursor) {
                    Some(i) => {
                        ...
                        self.sync_input();
                        self.status = ui::t!("whole_pivot_selected").into();
                    }
                    None => {
                        self.status = ui::t!("put_cursor_pivot").into();
                    }
                ...
                match self.pivot_at(self.cursor) {
                    ...
                    Some(i) => {
                        ...
                        if need_two && self.book.pivots[i].rows_sel.len() < 2 {
                            self.status =
                                ui::t!("works_pivots_2_row").into();
                        } else {
                            let d = &mut self.book.pivots[i];
                            let (name, on) = match id {
                                "pivot-totals" => {
                                    d.totals = !d.totals;
                                    (ui::t!("grand_totals"), d.totals)
                                }
                                "pivot-subtotals" => {
                                ...
                                "pivot-blank" => {
                                    ...
                                    (ui::t!("blank_rows"), d.blank_rows)
                                }
                                _ => {
                                    d.compact = !d.compact;
                                    (ui::t!("compact_form"), d.compact)
                                }
                            };
                            let d = self.book.pivots[i].clone();
                            self.dirty = true;
            ...
            "td-header" | "td-band-row" | "td-band-col" | "td-first" | "td-last" => {
                ...
                self.commit();
                if self.anchor.is_none() {
                    self.status = ui::t!("select_table_range").into();
                } else {
                    self.checkpoint();
                    let (a, b) = self.sel_rect();
                    ...
                    let what_ja = match id {
                        "td-header" => ui::t!("header_row"),
                        "td-band-row" => ui::t!("banded_rows"),
                        "td-band-col" => ui::t!("make_columns_banded"),
                        "td-first" => ui::t!("bold_first_column"),
                        _ => ui::t!("bold_last_column"),
                    };
                    self.status = if on {
                        ui::tf!("ctrl_z_undoes", a.a1(), b.a1(), what_ja)
                    } else {
                        // 塗りは剥がさない(掛ける前の姿を覚えていない)。
                        // **できないことを、できるように見せない**
                        ui::tf!("turned_off_table_colours", what_ja)
                    }
                    .into();
                }
            }
            ...
            "table-tpl" => {
                ...
                if self.anchor.is_none() {
                    ...
                    self.status = ui::t!("table_style_pick_one").into();
                }
            }
            ...
            "inssymbol" => {
                ...
                if !self.recent_symbols.is_empty() {
                    ...
                    items.push(("symbols:recent".into(), ui::tf!("recently_used", chars)));
                }
                for (key, label, chars) in symbol_groups() {
                    items.push((format!("symbols:{key}"), format!("{label}: {chars}")));
                }
                items.extend(menu(&[ui::item!("type_unicode_e_g")]));
                self.pick_kind = "symbol-group";
                self.pick_note = Some(ui::t!("symbols_pick_group_see").into());
                self.pick = Some((items, at));
            }
            ...
            "text-column" => {
                ...
                if self.anchor.is_none() {
                    self.status =
                        ui::t!("select_cells_split_text").into();
                } else {
                ...
                cx.spawn(async move |this, cx| {
                    ...
                    let _ = this.update(cx, |this, cx| {
                        match r {
                            ...
                            Some(Ok((p, mut other))) => {
                                this.checkpoint();
                                book::calc::recalc_all(&mut other);
                                let mut n = 0usize;
                                ...
                                this.dirty = true;
                                this.status = ui::tf!(
                                    "imported_sheets_values_no",
                                    n
                                )
                                .into();
                            }
                            Some(Err(e)) => this.status = ui::tf!("cant_import", e).into(),
                        }
                        cx.notify();
                    });
                })
            ...
            "scale" => {
                ...
                self.dirty = true;
                self.status = ui::tf!("print_scaling_applies_pdf", next).into();
            }
            ...
            "pagebreak" => {
                ...
                if has_row {
                    items.push((
                        "pagebreak:row".into(),
                        ui::tf!("remove_page_break_row", r + 1),
                    ));
                } else if r > 0 {
                    items.push((
                        "pagebreak:row".into(),
                        ui::tf!("start_new_sheet_row", r + 1),
                    ));
                }
                if has_col {
                    items.push((
                        "pagebreak:col".into(),
                        ui::tf!("remove_page_break_column", col_name(c)),
                    ));
                } else if c > 0 {
                    items.push((
                        "pagebreak:col".into(),
                        ui::tf!("start_new_sheet_column", col_name(c)),
                    ));
                }
                ...
                if n > 0 {
                    items.push((
                        "pagebreak:all".into(),
                        ui::tf!("remove_all_page_breaks", n),
                    ));
                }
                if items.is_empty() {
                    // A1 の上で、まだ1つも改ページが無いとき
                    self.status = ui::t!(
                        "page_break_where_paper"
                    )
                    .into();
                } else {
                    self.pick_kind = "pagebreak";
                    self.pick_note = Some(ui::t!("page_break_where_paper_ends").into());
                    self.pick = Some((items, at));
                }
            }
            ...
            "printarea-add" => {
                ...
                if self.anchor.is_none() {
                    self.status =
                        ui::t!("select_range_add_shift").into();
                } else {
                    self.checkpoint();
                    let range = self.sel_rect();
                    let sh = self.sheet_mut();
                    if sh.print_areas.contains(&range) {
                        self.undo_stack.pop();
                        self.status = ui::t!("range_already_print_area").into();
                    } else {
                        sh.print_areas.push(range);
                        let n = sh.print_areas.len();
                        self.dirty = true;
                        self.status = ui::tf!(
                            "added_print_area_areas",
                            range.0.a1(),
                            range.1.a1(),
                            n
                        )
            ...
            "read-only-rec" => {
                ...
                self.status = if self.book.read_only_rec {
                    ui::t!("suggests_view_only_whoever").into()
                } else {
                    ui::t!("no_longer_suggesting_read").into()
                };
            }
            ...
            "flash-fill" => {
                ...
                if col == 0 {
                    self.status =
                        ui::t!("source_column_left_nothing").into();
                    cx.notify();
                    return;
                }
                ...
                for r in a.row..=last {
                    ...
                    self.book.sheets[self.active].set(p, book::Cell::input(&v));
                    n += 1;
                }
                if n == 0 {
                    self.undo_stack.pop();
                    self.status = ui::t!("there_nothing_fill").into();
                } else {
                    self.dirty = true;
                    recalc_book(&mut self.book, self.active);
                    self.status = ui::tf!(
                        "filled_cells_match_example",
                        n
                    )
                    .into();
                }
            ...
            "paste-name" => {
                ...
                if items.is_empty() {
                    self.status = ui::t!(
                        "no_names_yet_formulas"
                    )
                    .into();
                } else {
                    self.pick_kind = "paste-name";
                    self.pick_note =
                        Some(ui::t!("name_put_formula_goes").into());
                    self.pick = Some((items, at));
                }
            }
            ...
            "autofit-col" | "autofit-row" => {
                ...
                self.dirty = true;
                self.status = if n == 0 {
                    ui::t!("nothing_there_theres_nothing").into()
                } else if col {
                    ui::tf!("fitted_columns_contents_ctrl", n).into()
                } else {
                    ui::tf!("fitted_rows_contents_ctrl", n).into()
                };
            }
            ...
            "show-left" => {
                ...
                self.status = if self.left_open {
                    ...
                    ui::t!("left_panel_closed").into()
                };
            }
            "show-right" => {
                self.right_open = !self.right_open;
                self.status = if self.right_open {
                    ui::t!("right_panel_cell_settings").into()
                } else {
                    ui::t!("right_panel_closed").into()
                };
            }
            "csv-kind" => {
                let at = self.pop_anchor();
                ...
                self.pick_note =
                    Some(ui::t!("character_encoding_separator_writing").into());
            ...
            "recover" => {
                ...
                if list.is_empty() {
                    ...
                    self.pick_note = Some(
                        ui::t!("backups_books_ended_without").into(),
                    );
                    // 控えのファイル名 — 訳さない
            ...
            "recover-every" => {
                ...
                self.pick_note =
                    Some(ui::t!("how_often_keep_auto").into());
                self.pick = Some((
                    menu(&[
                        ui::item!("off"),
                        ui::item!("every_minute"),
                        ui::item!("every_5_minutes"),
                        ui::item!("every_10_minutes"),
                    ]),
                    at,
                ));
            }
            ...
            "show-breaks" => {
                ...
                self.status = if self.show_breaks {
                    if r.is_empty() && c.is_empty() {
                        ui::t!("showing_where_paper_ends").into()
                    } else {
                        format!(
                    ...
                    ui::t!("page_break_lines_off").into()
                };
            }
            ...
            "fit-pages" => {
                ...
                self.pick_note = Some(
                    ui::t!("fit_paper_wins_over").into(),
                );
                self.pick = Some((
                    menu(&[
                        ui::item!("no_scaling"),
                        ui::item!("fit_all_columns_one"),
                        ui::item!("fit_all_rows_one"),
                        ui::item!("fit_sheet_one_page"),
                        ui::item!("2_pages_wide_1"),
                    ]),
                    at,
                ));
            }
            ...
            "printtitles" => {
                ...
                if self.anchor.is_some() {
                    ...
                    self.dirty = true;
                    self.status = ui::t!("title_rows_cleared").into();
                } else {
                    self.status =
                        ui::t!("select_rows_repeat_first").into();
                }
            }
            "print-gridlines" => {
                self.commit();
            ...
            "inssparkline" => {
                ...
                if self.anchor.is_none() {
                    self.status =
                        ui::t!("select_range_numbers_sparkline").into();
                } else {
                    // 本家と同じ3種から選ぶ(折れ線・縦棒・勝ち負け)
                    let at = self.pop_anchor();
                    self.pick_note = Some(ui::t!("sparkline_type").into());
            ...
            "insshape" => {
                ...
                self.pick_note = Some(ui::t!("shape_category").into());
                self.pick = Some((
                    menu(&[
                        ui::item!("basic_shapes"),
                        ui::item!("block_arrows"),
                        ui::item!("equation_shapes"),
                        ui::item!("flowchart"),
                        ui::item!("stars_ribbons"),
                        ui::item!("callouts"),
                        ui::item!("line_border"),
                    ]),
                    at,
            ...
            "dv-mark" => {
                ...
                if !self.dv_marks.is_empty() {
                    self.dv_marks.clear();
                    self.status = ui::t!("marks_cleared").into();
                    cx.notify();
                    return;
                }
                let si = self.active;
                ...
                if sh.validations.is_empty() {
                    self.status =
                        ui::t!("sheet_no_validation_rules")
                            .into();
                    cx.notify();
                    return;
                }
                ...
                self.status = if n == 0 {
                    ...
                    ui::tf!("marked_cells_press_again", n).into()
                };
            }
            "data-validation" => {
                self.commit();
            ...
            "defname" => {
                ...
                items.extend(menu(&[ui::item!("new_name_current_selection")]));
                self.pick_note = Some(
                    ui::t!("name_manager_pick_name").into(),
                );
            ...
            "pivot-chart" => {
                ...
                if let Some(at) = d.chart_at {
                    ...
                    self.dirty = true;
                    self.status = ui::t!("pivotchart_removed").into();
                    return;
                }
                // 置き場はピボットの下(表と重ならない所)
                ...
                self.status = ui::tf!(
                    "pivotchart_placed_refreshing_pivottable",
                    at.a1()
                )
                .into();
            }
            ...
            "forecast" => {
                ...
                self.status =
                    ui::t!("how_many_periods_ahead").into();
            }
            ...
            "scenario" => {
                ...
                let mut items: Vec<(String, String)> = self
                    ...
                    .collect();
                items.extend(menu(&[ui::item!("new_scenario_current_values")]));
                if !self.sheet().scenarios.is_empty() {
                    items.extend(menu(&[ui::item!("delete_scenario")]));
                }
                self.pick_note = Some(if self.sheet().scenarios.is_empty() {
                    ui::t!("there_none_yet_select").into()
                } else {
                    ui::t!("pressing_one_writes_values").into()
                });
            ...
            "split" => {
                if self.split.is_some() {
                    self.split = None;
                    self.status = ui::t!("split_removed").into();
                } else {
            ...
            "freeze" => {
                ...
                items.extend(menu(&[
                    ui::item!("freeze_cursor_rows_above"),
                    ui::item!("freeze_top_row"),
                    ui::item!("freeze_first_column"),
                ]));
                // 本家の「固定された枠に影を付ける」(viewtab:freezeshadow)。
                // **✓ は見出しだけ** — 鍵は素のまま(picks.rs がこの字で引き当てる)
                let (k, l) = ui::item!("shadow_frozen_edge");
                items.push((
                    k.to_string(),
            ...
            "fillparag" => {
                ...
                items.extend(menu(&[
                    ui::item!("other_type_rrggbb"),
                    // 単色以外(台帳 第2便の [中])。**色を選ぶとべた塗りに戻る** —
                    // 柄と虹はここから掛け直す
                    ui::item!("apply_pattern"),
                    ui::item!("gradient"),
                    ui::item!("pattern_background_colour"),
                ]));
                self.pick = Some((items, at));
            }
            // フォントの色。同じくパレット
            ...
            "custom-sort" => {
                ...
                let (rows, cols) = self.sheet().extent();
                if rows < 2 {
                    self.status = ui::t!("no_table_sort_needs").into();
                    return;
                }
            ...
            "rem-duplicates" => {
                ...
                let (rows, cols) = self.sheet().extent();
                if rows == 0 {
                    self.status = ui::t!("no_table").into();
                    return;
                }
            ...
            "currency" => {
                ...
                self.pick_note =
                    Some(ui::t!("currency_symbol_belongs_sheet").into());
            ...
            "datefmt" => {
                ...
                self.pick_note = Some(
                    ui::t!("date_style_choosing_one").into(),
                );
...
impl Calc {
    ...
    pub(crate) fn hide_lines(&mut self, id: &str) {
        ...
        if hide {
            ...
            if total > 0 && left == 0 {
                self.status = if cols {
                    ui::t!("cant_hide_every_column").into()
                } else {
                    ui::t!("cant_hide_every_row").into()
                };
                return;
            }
        }
        ...
        if n == 0 {
            ...
            self.status = match (hide, cols) {
                (true, true) => ui::t!("columns_already_hidden").into(),
                (true, false) => ui::t!("rows_already_hidden").into(),
                (false, true) => {
                    ui::t!("no_hidden_columns_selection").into()
                }
                (false, false) => {
                    ui::t!("no_hidden_rows_selection").into()
                }
            };
            return;
        }
        ...
        self.status = match (hide, cols) {
            (true, true) => {
                ui::tf!("hid_columns_select_across", n).into()
            }
            (true, false) => {
                ui::tf!("hid_rows_select_across", n).into()
            }
            (false, true) => ui::tf!("brought_back_columns", n).into(),
            (false, false) => ui::tf!("brought_back_rows", n).into(),
        };
    }
}
...
impl Calc {
    ...
    pub(crate) fn track_changes(&mut self) {
        match self.track_from.take() {
            None => {
                ...
                self.status = ui::t!(
                    "recording_changes_press_again"
                )
                .into();
            }
            Some(snap) => {
                let (who, when) = (crate::io::lock_identity(), crate::util::now_stamp());
                let mut add: Vec<book::ChangeRec> = Vec::new();
                ...
                self.status = if n == 0 {
                    ui::t!("stopped_recording_nothing_changed").into()
                } else {
                    ui::tf!("stopped_recording_entries_press", n)
                        .into()
                };
                if n == 0 {
                    self.show_changes();
    ...
    pub(crate) fn show_changes(&mut self) {
        if self.book.changes.is_empty() {
            self.status =
                ui::t!("no_change_history_yet").into();
            return;
        }
        let at = self.pop_anchor();
...
impl Calc {
    ...
    pub fn file_menu(&self) -> Vec<ui::filemenu::Item> {
        ...
        vec![
            ...
            I::new("f-csv", ui::t!("export_csv")),
            I::new("f-html", ui::t!("export_web")),
            // **形を選んで書き出す1つの入り口**
    ...
    pub fn file_menu_click(&mut self, id: &str, cx: &mut Context<Self>) {
        ...
        match id {
            ...
            "f-export" => {
                ...
                self.pick_note = Some(ui::t!("export_format").into());
                // **表の節から出せるのは4つ**(手引きの表)。
                // `.adoc` はここに出しません — 保存の側だからです
                self.pick = Some((
                    vec![
                        ("xlsx".into(), ui::t!("excel_workbook_xlsx").to_string()),
                        ("csv".into(), ui::t!("csv_csv").to_string()),
