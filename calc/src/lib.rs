...
pub struct Calc {
    ...
    /// 「パスワード表示/非表示アイコン」)
    pw_show: bool,
    /// 暗号化のパスワードの**1回目**。2回目と突き合わせてから掛けます。
    /// 打ち間違えたパスワードで包むと、**そのファイルは誰にも開けません** —
    /// 元に戻す手がないので、ここだけは2回聞きます
    pw_first: Option<String>,
    /// **このブックは壊れたファイルから拾い集めたもの**(開いて修復。
    /// 2026-08-22)。立っている間は**上書き保存を断ります** — いちばん怖い
    /// 事故は、穴あきに気づかず元の壊れたファイルを上書きすることです。
    /// 名前を付けて保存はできます(そちらは新しいファイルなので)
    salvaged: bool,
    /// 次に描くグラフの置き場(ピボットグラフが指す)。1回で消えます
    chart_dest: Option<Pos>,
    /// 拾い直しを聞いている間の控え(ファイルの道, 中身)
    repair_pend: Option<(std::path::PathBuf, Vec<u8>)>,
    /// 数式を値の代わりに出す(数式の表示)
    show_formulas: bool,
    ...
    freeze_shadow: bool,
    /// ウィンドウの分割(上の帯の行数・左の帯の列数)。固定と似ていますが、
    /// **帯の側もスクロールできる**のが違いです。離れた2か所を見比べるための
    /// もので、固定とは同時に使えません(片方を入れると相手を外します)。
    /// 画面だけの状態で、ファイルには残しません
    split: Option<Pos>,
    /// 分割した帯の先頭(帯そのもののスクロール位置)。固定の帯は必ず
    /// 先頭からですが、分割の帯は動きます
    split_view: Pos,
    ...
    /// **改ページプレビューの表示か。** 紙の切れ目を出して少し縮めます
    /// (2026-08-30)。標準との切り替えはリボンの「表示」の段です
    pub(crate) page_preview: bool,
    ...
    watch: Vec<(usize, Pos)>,
    /// **入力規則に合っていないセルの印**(2026-08-21 の D群)。(シート番号, セル)。
    ///
    /// 入力規則は*これから打つ字*を堰き止めますが、**先に入っていた値**や
    /// 貼り付けで入った値は素通りします。押したときに洗い出して丸で囲みます。
    /// もう一度押すと消えます(Excel は「マーク」と「クリア」の2つですが、
    /// 同じ物の入切なので1つにしました)。
    ///
    /// *ファイルには残しません。* その時の値を見た結果なので、開き直したら
    /// もう一度押す物です。
    dv_marks: Vec<(usize, Pos)>,
    ...
    /// 偽=このシートだけ / 真=このファイル(全シート)。
    /// フォルダ全体は別の道(`find_in_folder`)です
    pub(crate) find_book: bool,
