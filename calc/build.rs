...
//!
//! 絵の正本は `packaging/icons/officework-calc.svg` の1枚で、`.ico` は
...
fn main() {
    if std::env::var("CARGO_CFG_TARGET_OS").as_deref() == Ok("windows") {
        ...
        r.set_icon("../packaging/icons/officework-calc.ico");
        // 失敗しても組み立ては止めない — 絵が無いだけで、動く物は出来る
        if let Err(e) = r.compile() {
            println!("cargo:warning=絵を焼けませんでした: {e}");
        }
    }
    println!("cargo:rerun-if-changed=../packaging/icons/officework-calc.ico");
}
